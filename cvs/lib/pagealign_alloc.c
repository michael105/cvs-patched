...
static void new_memnode (void *aligned_ptr, info_t info) {
...
static info_t get_memnode (void *aligned_ptr) {
	info_t ret;
	memnode_t *c;
...
void *pagealign_alloc (size_t size) {
	void *ret;
#if HAVE_MMAP
...
void *pagealign_xalloc (size_t size) {
	void *ret;
...
void pagealign_free (void *aligned_ptr) {
#if HAVE_MMAP
