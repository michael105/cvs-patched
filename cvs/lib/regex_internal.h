...
static inline bool re_alloc_oversized (size_t n, size_t s) {
...
static inline bool re_x2alloc_oversized (size_t n, size_t s) {
	return BE ((SIZE_MAX / s - 1) / 2 < n, 0);
}

/*  Allocate an array of N objects, each with S bytes of memory,
    dynamically, with error checking.  S must be nonzero.  */
static inline void *re_xnmalloc (size_t n, size_t s) {
	return re_alloc_oversized (n, s) ? NULL : malloc (n * s);
}

/*  Change the size of an allocated block of memory P to an array of N
    objects each of S bytes, with error checking.  S must be nonzero.  */
static inline void *re_xnrealloc (void *p, size_t n, size_t s) {
...
/*  Reallocate a block of memory P to an array of (2 * (*PN) + 1)
    ...
    NULL.  */
static inline void *re_x2nrealloc (void *p, size_t *pn, size_t s) {
...
static inline void bitset_set (bitset set, Idx i) {
	set[i / BITSET_WORD_BITS] |= (bitset_word) 1 << i % BITSET_WORD_BITS;
}

static inline void bitset_clear (bitset set, Idx i) {
	set[i / BITSET_WORD_BITS] &= ~ ((bitset_word) 1 << i % BITSET_WORD_BITS);
}

static inline bool bitset_contain (const bitset set, Idx i) {
	return (set[i / BITSET_WORD_BITS] >> i % BITSET_WORD_BITS) & 1;
}

static inline void bitset_empty (bitset set) {
	memset (set, 0, sizeof (bitset));
}

static inline void bitset_set_all (bitset set) {
...
static inline void bitset_copy (bitset dest, const bitset src) {
