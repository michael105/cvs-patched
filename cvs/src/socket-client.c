...
static int socket_buffer_input(void *closure, char *data, size_t need,
                                size_t size,
                                size_t *got){
	...
	*got = 0;
	...
	do{
		...
		if ( nbytes < 0 ) 
			cvserr(1, 0, "reading from server: %s",
...
static int socket_buffer_output(void *closure, const char *data, size_t have,
                                 size_t *wrote){
	struct socket_buffer *sb = closure;
...
# ifdef SEND_NEVER_PARTIAL
	...
	if ( send(sb->socket, data, have, 0) < 0 ) 
		cvserr(1, 0, "writing to server socket: %s",
		        SOCK_STRERROR(SOCK_ERRNO));
# else
	while ( have > 0 ){
		int nbytes;
...
/*ARGSUSED*/
static int socket_buffer_flush(void *closure){
...
static int socket_buffer_shutdown(struct buffer *buf){
	...
	if ( buf->input ){
		...
		if ( ! buf_empty_p(buf ) 
			...
			cvserr(0, 0, "dying gasps from %s unexpected",
			        current_parsed_root->hostname);
		else if ( err == -1 ) 
			cvserr(0, 0, "reading from %s: %s",
			        current_parsed_root->hostname,
			        SOCK_STRERROR(SOCK_ERRNO));
...
# endif
			if ( shutdown(n->socket, 0) < 0 ){
				cvserr(1, 0, "shutting down server socket: %s",
				        SOCK_STRERROR(SOCK_ERRNO));
			}
