...
const char *get_cvs_tmp_dir(void){
	...
	if(!retval
	        || !*retval)cvserr(1, 0, "No temp dir specified.");
...
static const char * const* cmd_synonyms(void){
	char ** synonyms;
	char ** line;
...
unsigned long int lookup_command_attribute(const char *cmd_name){
	...
	if ( !cm->fullname )
		cvserr(1, 0, "unknown command: %s", cmd_name);
	return cm->attr;
}
...
static RETSIGTYPE main_cleanup(int sig){
	...
	cvserr(1, 0, "received %s signal", name);
...
static unsigned int divide_by(unsigned char buf[COMMITID_RAW_SIZE],
                               unsigned int d){
...
static void convert(char const input[COMMITID_RAW_SIZE], char *output){
...
cause intermittent sandbox corruption.");
	...
	if ( help ){
		...
		if ( (cp = getenv(CVSUMASK_ENV)) != NULL ){
			...
			if ( *end != '\0' ) 
				cvserr(1, errno, "invalid umask value in %s (%s)",
				        CVSUMASK_ENV, cp);
		}
...
# endif /* AUTH_SERVER_SUPPORT || HAVE_GSSAPI */
...
#endif
		{
			...
			CurDir = xgetcwd();
			if ( CurDir == NULL ) 
				cvserr(1, errno, "cannot get working directory");
		}
		...
		*/
		if ( !server_active ){
			...
			if ( CVSroot_cmdline ){
				if ( !(CVSroot_parsed = parse_cvsroot(CVSroot_cmdline)) ) 
					cvserr(1, 0, "Bad CVSROOT: `%s'.", CVSroot_cmdline);
			}
			...
			*/
			...
			if ( !CVSroot_parsed ){
				if ( !(CVSroot_parsed = parse_cvsroot(CVSROOT_DFLT)) ) 
					cvserr(1, 0, "Bad CVSROOT: `%s'.", CVSROOT_DFLT);
			}
#endif /* CVSROOT_DFLT */
		...
		if ( CVSroot_parsed ){
			...
			if ( addnode(root_directories, n) ) 
				cvserr(1, 0, "cannot add initial CVSROOT %s", n->key);
		}
...
char *Make_Date(const char *rawdate){
	...
	if ( !get_date(&t, rawdate, NULL) ) 
		cvserr(1, 0, "Can't parse date/time: `%s'", rawdate);
...
void date_to_tm(struct tm *dest, const char *source){
	if(sscanf (source, SDATEFORM,
		...
		cvserr(0, 0, "internal error: bad date %s", source);
...
*/
static char *format_time_t(time_t unixtime){
...
/*  Format a date in the local timezone using format_time_t() given a date from
    ...
    RETURNS
     ...
     otherwise.
*/
char *format_date_alloc(char *datestr){
	struct timespec t;
	char *buf;
	...
	if ( !get_date(&t, datestr, NULL) ){
		cvserr(0, 0, "Can't parse date/time: `%s'.", datestr);
		goto as_is;
	}
	...
	*/
	if ( (buf = format_time_t(t.tv_sec)) == NULL ){
		cvserr(0, 0, "Unable to reformat date `%s'.", datestr);
		goto as_is;
	}
...
void usage(register const char *const *cpp){
