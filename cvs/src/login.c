...
static char *construct_cvspass_filename(void){
	...
	if ( ! homedir ){
		/*  FIXME?  This message confuses a lot of users, at least
		    ...
		    memory only)...  */
		cvserr(1, 0, "could not find out home directory");
		return NULL;
	}
...
*/
static char *password_entry_parseline(const char *cvsroot_canonical,
	...
	if ( *linebuf == '/' ){
		...
		*/
		...
		if ( p == NULL ){
			if ( warn && !really_quiet ) 
				cvserr(0, 0,
				        "warning: skipping invalid entry in password file at line %d", linenumber);
			return NULL;;
		}
...
static char *password_entry_operation(password_entry_operation_t operation,
                                       cvsroot_t *root, char *newpassword){
	char *passfile;
	FILE *fp;
...
internal error: can only call password_entry_operation with pserver method");
		cvserr(1, 0, "CVSROOT: %s", root->original);
	}
	...
	*/
	...
	if ( fp == NULL ){
		cvserr(0, errno, "warning: failed to open %s for reading",
		        passfile);
		goto process;
	}
	...
	if ( line_length < 0 && !feof(fp) ){
		cvserr(0, errno, "cannot read %s", passfile);
		goto error_exit;
	}
	if ( fclose(fp) < 0 ) 
		/* not fatal, unless it cascades */
		cvserr(0, errno, "cannot close %s", passfile);
	fp = NULL;
...
process:
	...
	*/
	if(!noexec && password != NULL && (operation == password_entry_delete
		...
		fp = CVS_FOPEN(passfile, "r");
		if ( fp == NULL ) 
			cvserr(1, errno, "failed to open %s for reading", passfile);

		/* create and open a temp file */
		if ( (tmp_fp = cvs_temp_file(&tmp_name)) == NULL ) 
			cvserr(1, errno, "unable to open temp file %s", tmp_name);
		...
		while ( (line_length = getline(&linebuf, &linebuf_len, fp)) >= 0 ){
			...
			if(line < found_at
				...
				if ( fprintf(tmp_fp, "%s", linebuf) == EOF ){
					...
					cvserr(0, errno, "fatal error: cannot write %s", tmp_name);
					if ( fclose(tmp_fp) == EOF ) 
						cvserr(0, errno, "cannot close %s", tmp_name);
					/*  call CVS_UNLINK instead of unlink_file since the file
					    got created in noexec mode
					*/
					if ( CVS_UNLINK(tmp_name) < 0 ) 
						cvserr(0, errno, "cannot remove %s", tmp_name);
	...
	*/
	if(!noexec && operation == password_entry_add
		...
		if ( (fp = CVS_FOPEN(passfile, "a")) == NULL ) 
			cvserr(1, errno, "could not open %s for writing", passfile);

		if ( fprintf(fp, "/1 %s %s\n", cvsroot_canonical, newpassword) == EOF ) 
			cvserr(1, errno, "cannot write %s", passfile);
		if ( fclose(fp) < 0 ) 
			cvserr(1, errno, "cannot close %s", passfile);
	}
...
error_exit:
	...
	if ( operation != password_entry_lookup ) 
		cvserr(1, 0, "fatal error: exiting");
...
int login(int argc, char **argv){
	...
	if ( current_parsed_root->method != pserver_method ){
		...
		cvserr(1, 0, "CVSROOT: %s", current_parsed_root->original);
	}
	...
	if ( current_parsed_root->password ){
		...
		*/
		if ( !tmp )cvserr(1, errno, "login: Failed to read password.");
...
char *get_cvs_password(void){
	...
	if ( getenv("CVS_PASSWORD") != NULL ){
		/*  In previous versions of CVS one could specify a password in
		    ...
		    it, of course).
		*/
		cvserr(0, 0, "CVS_PASSWORD is no longer supported; ignored");
	}
