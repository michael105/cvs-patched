...
/*  Force the existance of lb->log.
    ...
    ERRORS
     Errors creating the log file will output a message viacvserr().  Whether
     the error is fatal or not is dependent on lb->fatal_errors.
*/
static inline void log_buffer_force_file(struct log_buffer *lb){
	lb->log = cvs_temp_file(&lb->back_fn);
	if ( !lb->log )
		cvserr(lb->fatal_errors, errno, "failed to open log file.");
}
#endif /* PROXY_SUPPORT */
...
static int log_buffer_input(void *closure, char *data, size_t need,
                             size_t size,
                             size_t *got){
...
static int log_buffer_output(void *closure, const char *data, size_t have,
                              size_t *wrote){
	struct log_buffer *lb = closure;
	int status;
...
static int log_buffer_flush(void *closure){
	...
	*/
	if ( lb->log && (fflush(lb->log)) ) 
		cvserr(0, errno, "flushing log file");
...
static int log_buffer_block(void *closure, bool block){
	struct log_buffer *lb = closure;
...
log_buffer_rewind(struct buffer *buf){
	...
	if ( lb->log ){
		...
		fd = dup(fileno (tmp));
		if ( fclose(tmp) < 0 ) 
			cvserr(0, errno, "closing log file");
	} else
		fd = open(DEVNULL, O_RDONLY);

	/* Catch dup/open errors.  */
	if ( fd < 0 ){
		cvserr(lb->fatal_errors, errno, "failed to rewind log buf.");
		return NULL;
	}
...
log_buffer_closelog(struct buffer *buf){
	...
	if ( lb->back_fn ){
		...
		lb->back_fn = NULL;
		if ( CVS_UNLINK(tmp) ) 
			cvserr(0, errno, "Failed to delete log file.");
		free(tmp);
	}
...
static int log_buffer_get_fd(void *closure){
	struct log_buffer *lb = closure;
...
static int log_buffer_shutdown(struct buffer *buf){
	struct log_buffer *lb = buf->closure;
...
void setup_logfiles(char *var, struct buffer **to_server_p,
	...
	*/
	if ( log ){
		...
		fp = fopen(buf, "wb");
		if ( !fp )
			cvserr(0, errno, "opening to-server logfile %s", buf);
		else
...
# endif /* PROXY_SUPPORT */
		...
		fp = fopen(buf, "wb");
		if ( !fp )
			cvserr(0, errno, "opening from-server logfile %s", buf);
		else
