...
static int onoff_filesdoneproc(void *callerdat, int err,
...
static int ncheck_fileproc(void *callerdat, struct file_info *finfo){
	...
	if ( fp == NULL ){
		if ( !existence_error(errno) ) 
			cvserr(0, errno, "cannot open %s", CVSADM_NOTIFY);
		return 0;
	}
...
static int edit_fileproc(void *callerdat, struct file_info *finfo){
	...
	if ( !vers->vn_user ){
		cvserr(0, 0, "no such file %s; ignored", finfo->fullname);
		return 1;
	}
		...
		if ( editors ){
			if ( check_edited ){
				...
				*/
				...
				if ( !really_quiet )
					cvserr(0, 0, "Skipping file `%s' due to existing editors.",
					        finfo->fullname);
				return 1;
			}
			free(editors);
	...
	if ( fclose(fp) < 0 ){
		if ( finfo->update_dir[0] == '\0' ) 
			...
			cvserr(0, errno, "cannot close %s/%s", finfo->update_dir,
			        CVSADM_NOTIFY);
	}
		...
		if ( node != NULL ){
			...
			if ( baserev == NULL ){
				/*  This can only happen if the CVS/Baserev file got
				    ...
				    that.  */
				cvserr(0, 0, "%s not mentioned in %s", finfo->fullname,
				        CVSADM_BASEREV);
				...
				cvserr(0, 0, "run update to complete the unedit");
				return 0;
			}
...
void mark_up_to_date(const char *file){
	...
	if ( unlink_file(base) < 0 && ! existence_error (errno) ) 
		cvserr(0, errno, "cannot remove %s", file);
	free(base);
}
...
static int notify_proc(const char *repository, const char *filter,
	...
	*/
	...
	if ( pipefp == NULL ){
		cvserr(0, errno, "cannot write entry to notify filter: %s",
		        cmdline);
		free(cmdline);
		return 1;
	}
...
/*  FIXME: this function should have a way to report whether there was
    ...
    to the client.  */
void notify_do(int type, const char *filename, const char *update_dir,
	...
	*/
	...
	case 'E':
		if ( strpbrk(val, ",>;=\n") != NULL ){
			cvserr(0, 0, "invalid character in editor value");
			return;
		}
	...
	while ( p != NULL ){
		...
		if ( notif != NULL ){
			...
			if ( fp == NULL && !existence_error(errno) ) 
				cvserr(0, errno, "cannot read %s", usersname);
			if ( fp != NULL ){
				while ( getline(&line, &line_len, fp) >= 0 ){
				...
				if ( ferror(fp) ) 
					cvserr(0, errno, "cannot read %s", usersname);
				if ( fclose(fp) < 0 ) 
					cvserr(0, errno, "cannot close %s", usersname);
			}
			free(usersname);
			if ( line != NULL ) 
				free(line);
