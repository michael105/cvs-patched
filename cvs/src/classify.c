...
Ctype Classify_File(struct file_info *finfo, char *tag, char *date,
	...
	if ( vers->vn_user == NULL ){
		...
		if ( vers->vn_rcs == NULL ){
			...
			if ( vers->ts_user == NULL ){
				...
				if ( !force_tag_match || !(vers->tag || vers->date) ) 
					if ( !really_quiet )
						cvserr(0, 0, "nothing known about `%s'",
						        finfo->fullname);
				ret = T_UNKNOWN;
			} else{
		...
		if ( vers->ts_user == NULL ){
			...
			if ( !really_quiet )
				cvserr(0, 0, "`%s' should be removed and is still there",
				        finfo->fullname);
			ret = T_REMOVED;
		}
	} else{
		...
		if ( vers->vn_rcs == NULL || RCS_isdead(vers->srcfile, vers->vn_rcs) ){
			...
			if ( vers->ts_user == NULL ){
				...
				if ( !really_quiet )
					cvserr(0, 0, "warning: `%s' is not (any longer) pertinent",
					        finfo->fullname);
				ret = T_REMOVE_ENTRY;
				...
				*/
				if ( !really_quiet )
					cvserr(0, 0, "`%s' is no longer in the repository",
					        finfo->fullname);
				ret = T_REMOVE_ENTRY;
			}
			...
			if ( vers->ts_user == NULL ){
				...
				*/
				...
				if ( strcmp(cvs_cmd_name, "update") == 0 ) 
					if ( !really_quiet )
						cvserr(0, 0, "warning: `%s' was lost", finfo->fullname);
				ret = T_CHECKOUT;
