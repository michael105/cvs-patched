...
void copy_file(const char *from, const char *to){
	...
	if ( isdevice(from) ){
		...
		if ( stat(from, &sb) < 0 ) 
			cvserr(1, errno, "cannot stat %s", from);
		mknod(to, sb.st_mode, sb.st_rdev);
#else
		cvserr(1, 0, "cannot copy device files on this system (%s)",
		        from);
#endif
	} else{
		/* Not a link or a device... probably a regular file. */
		if ( (fdin = open(from, O_RDONLY)) < 0 ) 
			cvserr(1, errno, "cannot open %s for copying", from);
		if ( fstat(fdin, &sb) < 0 ) 
			cvserr(1, errno, "cannot fstat %s", from);
		if ( (fdout = creat(to, (int) sb.st_mode & 07777)) < 0 ) 
			cvserr(1, errno, "cannot create %s for copying", to);
		if ( sb.st_size > 0 ){
			char buf[BUFSIZ];
			int n;
			...
			for(;;){
				...
				if ( n == -1 ){
					...
					if ( errno == EINTR ) 
						continue;
#endif
					cvserr(1, errno, "cannot read file %s for copying", from);
				} else if ( n == 0 ) 
					break;

				if ( write(fdout, buf, n) != n ){
					cvserr(1, errno, "cannot write file %s for copying", to);
				}
			}
		}

		if ( close(fdin) < 0 ) 
			cvserr(0, errno, "cannot close %s", from);
		if ( close(fdout) < 0 ) 
			cvserr(1, errno, "cannot close %s", to);
	}
...
/*
    ...
    bits set.
*/
bool isaccessible(const char *file, int mode){
//#ifdef SETXID_SUPPORT
// whether the real uid/gid does have the access rights, is uninteresting imho. miSc
#if 1
	struct stat sb;
	int umask = 0;
	int gmask = 0;
	...
	if ( uid == sb.st_uid )
		mode <<= 6;
	else	if ( sb.st_gid == getegid() ) //group
		mode <<= 3;
	...
	*/
	...
	return access(file, mode) ==
	       0; // fetches access rights for the real uid/gid(!)
	// miSc(linux)
...
void make_directory(const char *name){
	...
	if ( stat(name, &sb) == 0 && (!S_ISDIR (sb.st_mode)) ) 
		cvserr(0, 0, "%s already exists but is not a directory",
		        name);
	if ( !noexec && mkdir(name, 0777) < 0 ) 
		cvserr(1, errno, "cannot make directory %s", name);
}
...
void make_directories(const char *name){
	...
	if ( ! existence_error(errno) ){
		cvserr(0, errno, "cannot make path to %s", name);
		return;
	}
...
int mkdir_if_needed(const char *name){
	if ( mkdir(name, 0777) < 0 ){
		...
		if ( save_errno != EEXIST && !isdir(name) ) 
			cvserr(1, save_errno, "cannot make directory %s", name);
		return 1;
	}
	return 0;
}
...
void xchmod(const char *fname, int writable){
	...
	if ( stat(fname, &sb) < 0 ){
		if ( !noexec )
			cvserr(0, errno, "cannot stat %s", fname);
		return;
	}
	oumask = umask(0);
	(void) umask(oumask);
	...
	if ( chmod(fname, mode) < 0 ) 
		cvserr(0, errno, "cannot change mode of file %s", fname);
	// miSc add chgrp here
}
...
void rename_file(const char *from, const char *to){
	...
	if ( rename(from, to) < 0 ) 
		cvserr(1, errno, "cannot rename file %s to %s", from, to);
}
...
int xcmp(const char *file1, const char *file2){
	...
	if ( lstat(file1, &sb1) < 0 ) 
		cvserr(1, errno, "cannot lstat %s", file1);
	if ( lstat(file2, &sb2) < 0 ) 
		cvserr(1, errno, "cannot lstat %s", file2);
...
#else
		cvserr(1, 0,
		        "cannot compare device files on this system (%s and %s)",
		        file1, file2);
#endif
	}

	if ( (fd1 = open(file1, O_RDONLY)) < 0 ) 
		cvserr(1, errno, "cannot open file %s for comparing", file1);
	if ( (fd2 = open(file2, O_RDONLY)) < 0 ) 
		cvserr(1, errno, "cannot open file %s for comparing", file2);
	...
	if ( sb1.st_size != sb2.st_size ) 
		...
		do{
			...
			if ( read1 == (size_t)-1 ) 
				cvserr(1, errno, "cannot read file %s for comparing", file1);

			read2 = block_read(fd2, buf2, buf_size);
			if ( read2 == (size_t)-1 ) 
				cvserr(1, errno, "cannot read file %s for comparing", file2);
...
char *cvs_temp_name(void){
	...
	fp = cvs_temp_file(&fn);
	if ( fp == NULL ) 
		cvserr(1, errno, "Failed to create temporary file");
	if ( fclose(fp) == EOF ) 
		cvserr(0, errno, "Failed to close temporary file %s", fn);
	return fn;
}
	...
	if ( fd == -1 ) 
		...
		*/
		...
		if ( close(fd) ) 
			cvserr(0, errno, "Failed to close temporary file %s", fn);
		if ( CVS_UNLINK(fn) ) 
			cvserr(0, errno, "Failed to unlink temporary file %s", fn);
		errno = save_errno;
	}
...
*/
char *strcat_filename_onto_homedir(const char *dir, const char *file){
...
void expand_wild(int argc, char **argv, int *pargc, char ***pargv){
	...
	if ( size_overflow_p(xtimes (argc, sizeof (char *))) ){
		*pargc = 0;
		*pargv = NULL;
		cvserr(0, 0, "expand_wild: too many arguments");
		return;
	}
	*pargc = argc;
