...
*/
static FILE *setup_update_command(pid_t *child_pid){
	int tofd, fromfd;
...
static int close_update_command(FILE *fp, pid_t child_pid){
	int status;
	pid_t w;
	...
	if ( w == -1 ) 
		cvserr(1, errno, "waiting for process %d", child_pid);
	...
	for(i = arg_start_idx; i < argc; i++){
		...
		if ( isdir(thisarg) ){
			if ( CVS_CHDIR(thisarg) < 0 ){
				if ( !really_quiet )
					cvserr(0, errno, "can't chdir to: %s", thisarg);
				continue;
			}
			if ( !isdir(CVSADM) ){
				if ( !really_quiet )
					cvserr(0, 0, "no repository directory: %s", thisarg);
				if ( restore_cwd(&cwd) ) 
					cvserr(1, errno,
			...
			if ( !really_quiet )
				cvserr(0, 0, "no such directory: %s", thisarg);
			continue;
		}
		...
		if ( !really_quiet ){
			...
			if ( fp == NULL ){
				cvserr(0, 0, "cannot run command:");
				run_print(stderr);
				cvserr(1, 0, "Exiting due to fatal error referenced above.");
			}
			...
			if ( line_length < 0 && !feof(fp) ) 
				cvserr(0, errno, "cannot read from subprocess");
			...
			if ( status != 0 ){
				cvserr(0, 0, "unable to release `%s' (%d)", thisarg, status);
				if ( restore_cwd(&cwd) ) 
					cvserr(1, errno,
					        "Failed to restore current directory, `%s'.",
					        cwd.name);
		...
		if ( delete_flag ){
			...
			if ( unlink_file_dir(thisarg) < 0 ) 
				cvserr(0, errno, "deletion of directory %s failed", thisarg);
		}
