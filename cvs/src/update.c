...
int update(int argc, char **argv){
	...
	while ( (c = getopt(argc, argv, "+ApCPflRQqduk:r:D:j:I:W:")) != -1 ){
		...
		case 'j':
			if ( join_orig2 )
				cvserr(1, 0, "only two -j options can be specified");
			if ( join_orig1 ){
			...
			if ( failed_patches_count == 0 ){
				...
				for(i = 0; i < failed_patches_count; i++)
					if ( unlink_file(failed_patches[i] )  < 0
						...
						cvserr(0, errno, "cannot remove %s",
						        failed_patches[i]);
...
*/
int do_update(int argc, char **argv, char *xoptions, char *xtag, char *xdate,
...
static int get_linkinfo_proc(void *callerdat, struct file_info *finfo){
	char *fullpath;
	Node *linkp;
...
static void update_ignproc(const char *file, const char *dir){
...
static int update_filesdone_proc(void *callerdat, int err,
	...
	if ( strcmp(cvs_cmd_name, "export") == 0 ){
		...
		if ( unlink_file_dir(CVSADM) < 0 && !existence_error (errno) ) 
			cvserr(0, errno, "cannot remove %s directory", CVSADM);
...
static Dtype update_dirent_proc(void *callerdat, const char *dir,
	...
	if ( !isdir(dir) ){
		...
		if ( noexec ){
			...
			cvserr(0, 0, "New directory `%s' -- ignored", update_dir);
			return R_SKIP_ALL;
		} else{
	...
	*/
	if ( !pipeout ){
		if ( update_build_dirs ){
			...
			if ( unlink_file(tmp) < 0 && ! existence_error (errno) ) 
				cvserr(1, errno, "cannot remove file %s", tmp);
#ifdef SERVER_SUPPORT
			if ( server_active )
	...
	/* print the warm fuzzy message */
	if ( !quiet )
		cvserr(0, 0, "Updating %s", update_dir);
...
*/
/* ARGSUSED */
static int update_dirleave_proc(void *callerdat, const char *dir, int err,
                                 const char *update_dir, List *entries){
	...
	if ( strchr(dir, '/') == NULL ){
		...
		if ( update_prune_dirs && isemptydir(dir, 0) ){
			...
			if ( unlink_file_dir(dir) < 0 && !existence_error (errno) ) 
				cvserr(0, errno, "cannot remove %s directory", dir);
...
int isemptydir(const char *dir, int might_not_exist){
	...
	if ( (dirp = CVS_OPENDIR(dir)) == NULL ){
		if ( might_not_exist && existence_error(errno) ) 
			return 0;
		cvserr(0, errno, "cannot open directory %s for empty check",
		        dir);
		return 0;
	}
	errno = 0;
	while ( (dp = CVS_READDIR(dirp)) != NULL ){
		if ( strcmp(dp->d_name, "." )  != 0
			...
			if ( strcmp(dp->d_name, CVSADM) != 0 ){
				...
				if ( save_cwd(&cwd) ) 
					cvserr(1, errno, "Failed to save current directory.");

				if ( CVS_CHDIR(dir) < 0 ) 
					cvserr(1, errno, "cannot change directory to %s", dir);
	...
	if ( errno != 0 ){
		cvserr(0, errno, "cannot read directory %s", dir);
		(void) CVS_CLOSEDIR(dirp);
		return 0;
	}
	(void) CVS_CLOSEDIR(dirp);
...
static int scratch_file(struct file_info *finfo, Vers_TS *vers){
	...
	if ( unlink_file(finfo->file) < 0 && ! existence_error (errno) ) 
		cvserr(0, errno, "unable to remove %s", finfo->fullname);
		...
		if ( isfile(finfo->file) ) 
			...
			if ( unlink_file_dir(backup) < 0 ){
				...
				if ( !existence_error(errno) ) 
					...
					cvserr(0, errno, "error removing %s", backup);
			}
			free(backup);
			backup = NULL;
		}
	...
	if ( file_is_dead || status == 0 ){
		...
		if ( !pipeout ){
			...
			if ( file_is_dead ){
				...
				if ( unlink_file(finfo->file) < 0 && ! existence_error (errno) ){
					cvserr(0, errno, "cannot remove %s", finfo->fullname);
				}
			} else
				Register(finfo->entries, finfo->file,
		...
		cvserr(0, 0, "could not check out %s", finfo->fullname);
...
/*  Patch a file.  Runs diff.  This is only done when running as the
    ...
    itself.
*/
static int patch_file(struct file_info *finfo, Vers_TS *vers_ts,
                       int *docheckout,
	...
	*docheckout = 0;
	...
	if ( isfile(finfo->file) ) 
		...
		if ( unlink_file(backup )  < 0
		        && !existence_error(errno))
			cvserr(0, errno, "cannot remove %s", backup);
	}
	...
	e = CVS_FOPEN(file1, "w");
	if ( e == NULL ) 
		cvserr(1, errno, "cannot open %s", file1);
	...
	*/
	...
	if ( fclose(e) < 0 ) 
		cvserr(1, errno, "cannot close %s", file1);
	...
	if ( ! fail ){
		e = CVS_FOPEN(file2, "w");
		if ( e == NULL ) 
			cvserr(1, errno, "cannot open %s", file2);
		...
		if ( fclose(e) < 0 ) 
			cvserr(1, errno, "cannot close %s", file2);
	...
	if ( !fail ){
		...
		if ( stat(file2, &file2_info) < 0 ) 
			cvserr(1, errno, "could not stat %s", file2);
		if ( stat(finfo->file, file_info) < 0 ) 
			cvserr(1, errno, "could not stat %s", finfo->file);
...
# define BINARY "Binary"
		...
		if ( e == NULL ) 
			cvserr(1, errno, "could not open diff output file %s",
			        finfo->fullname);
	...
	if ( ! fail ){
		...
		if ( stat(vers_ts->srcfile->path, file_info) < 0 ) 
			cvserr(1, errno, "could not stat %s",
			        vers_ts->srcfile->path);
		if(chmod (finfo->file,
		...
		if ( retcode != 0 && retcode != 1 ) 
			cvserr(retcode == -1 ? 1 : 0, retcode == -1 ? old_errno : 0,
	...
	if ( unlink_file(file1 )  < 0
	        && !existence_error(errno))
		cvserr(0, errno, "cannot remove %s", file1);
	if ( unlink_file(file2 )  < 0
	        && !existence_error(errno))
		cvserr(0, errno, "cannot remove %s", file2);
...
static void patch_file_write(void *callerdat, const char *buffer, size_t len){
	...
	if ( fwrite(buffer, 1, len, data->fp) != len ) 
		cvserr(1, errno, "cannot write %s", data->filename);
...
static void RegisterMerge(struct file_info *finfo, Vers_TS *vers,
...
static int merge_file(struct file_info *finfo, Vers_TS *vers){
	...
	*/
	...
	if ( unlink_file(backup) && !existence_error (errno) ) 
		cvserr(0, errno, "unable to remove %s", backup);
	...
	if ( strcmp(vers->options, "-kb" )  == 0
		...
		if ( status ){
			cvserr(0, 0, "failed to check out `%s' file",
			        finfo->fullname);
			cvserr(0, 0, "restoring `%s' from backup file `%s'",
		...
		cvserr(0, 0, "nonmergeable file needs merge");
		cvserr(0, 0, "revision %s from repository is now in %s",
		        vers->vn_rcs, finfo->fullname);
		cvserr(0, 0, "file from working directory is now in %s",
		        backup);
		write_letter(finfo, 'C');
	...
	if ( status == 1 ){
		cvserr(0, 0, "conflicts found in %s", finfo->fullname);
...
static void join_file(struct file_info *finfo, Vers_TS *vers){
	...
	if ( rev2 == NULL || RCS_isdead(vers->srcfile, rev2) ){
		...
		*/
		...
		if ( unlink_file(finfo->file) < 0 && ! existence_error (errno) ) 
			cvserr(0, errno, "cannot remove file %s", finfo->fullname);
#ifdef SERVER_SUPPORT
		if ( server_active )
...
#endif
		if ( ! really_quiet ) 
			cvserr(0, 0, "scheduling `%s' for removal", finfo->fullname);
...
int special_file_mismatch(struct file_info *finfo, char *rev1, char *rev2){
#ifdef PRESERVE_PERMISSIONS_SUPPORT
	struct stat sb;
	RCSVers *vp;
	Node *n;
...
# ifdef HAVE_STRUCT_STAT_ST_RDEV
			if ( lstat(finfo->file, &sb) < 0 ) 
				cvserr(1, errno, "could not get file information for %s",
				        finfo->file);
			rev1_uid = sb.st_uid;
			rev1_gid = sb.st_gid;
...
# else
			cvserr(1, 0,
			        "cannot handle device files on this system (%s)",
			        finfo->file);
# endif
		}
		...
		if ( n != NULL ) 
			...
			if ( n == NULL ) 
				...
				if ( strcmp(ftype, "character") == 0 ) 
					...
					cvserr(0, 0, "%s:%s unknown file type `%s'",
...
# endif
	...
	if ( rev1_symlink != NULL && rev2_symlink == NULL ){
		cvserr(0, 0, "%s is a symbolic link",
		...
		if ( check_uids && rev1_uid != rev2_uid ){
			cvserr(0, 0, "%s: owner mismatch between %s and %s",
			        finfo->file,
			        (rev1 == NULL ? "working file" : rev1),
			        (rev2 == NULL ? "working file" : rev2));
			result = 1;
		...
		if ( check_gids && rev1_gid != rev2_gid ){
			cvserr(0, 0, "%s: group mismatch between %s and %s",
			        finfo->file,
			        (rev1 == NULL ? "working file" : rev1),
			        (rev2 == NULL ? "working file" : rev2));
			result = 1;
		...
		if(check_modes &&
			...
			cvserr(0, 0, "%s: permission mismatch between %s and %s",
			        finfo->file,
			        (rev1 == NULL ? "working file" : rev1),
			        (rev2 == NULL ? "working file" : rev2));
			result = 1;
		...
		if ( (rev1_mode & S_IFMT) != (rev2_mode & S_IFMT) ){
			cvserr(0, 0, "%s: %s and %s are different file types",
			        finfo->file,
			        (rev1 == NULL ? "working file" : rev1),
			        (rev2 == NULL ? "working file" : rev2));
			result = 1;
			...
			if ( rev1_dev != rev2_dev ){
				cvserr(0, 0, "%s: device numbers of %s and %s do not match",
				        finfo->file,
				        (rev1 == NULL ? "working file" : rev1),
				        (rev2 == NULL ? "working file" : rev2));
				result = 1;
		...
		if ( compare_linkage_lists(rev1_hardlinks, rev2_hardlinks) == 0 ){
			cvserr(0, 0, "%s: hard linkage of %s and %s do not match",
			        finfo->file,
			        (rev1 == NULL ? "working file" : rev1),
			        (rev2 == NULL ? "working file" : rev2));
			result = 1;
