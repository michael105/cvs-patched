...
int Checkin(int type, struct file_info *finfo, char *rev, char *tag,
		...
		if ( tocvsPath ){
			if ( unlink_file_dir(finfo->file) < 0 ) 
				if ( ! existence_error(errno) ) 
					cvserr(1, errno, "cannot remove %s", finfo->fullname);
	...
	*/
	...
	case 0:			/* everything normal */
		...
		if((1
			...
			if(RCS_checkout (finfo->rcs, finfo->file, rev, NULL,
				...
				cvserr(1, 0, "failed when checking out new copy of %s",
				        finfo->fullname);
			xchmod(finfo->file, 1); //miSc todo set grup here (?)
			set_time = 1;
		}
	...
	case -1:			/* fork failed */
		...
		if ( !noexec )
			cvserr(1, errno, "could not check in %s -- fork failed",
			        finfo->fullname);
		return(1);
	...
	default:			/* ci failed */
		...
		if ( !noexec )
			cvserr(0, 0, "could not check in %s", finfo->fullname);
		return(1);
	}
