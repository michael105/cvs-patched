...
int ls(int argc, char **argv){
	...
	if ( current_parsed_root->isremote ){
		...
		if ( is_rls ? !(supported_request ("rlist") || supported_request ("ls") ) 
			...
			cvserr(1, 0, "server does not support %s", cvs_cmd_name);
...
static int ls_print_dir(Node *p, void *closure){
...
*/
static void long_format_data_delproc(Node *n){
...
static int ls_fileproc(void *callerdat, struct file_info *finfo){
	...
	if ( regexp_match ){
		...
		if ( (regex_err = re_comp(regexp_match)) != NULL ){
			cvserr(1, 0, "bad regular expression passed to 'ls': %s",
			        regex_err);
		}
...
/*
    ...
    recursing or not.
*/
static Dtype ls_direntproc(void *callerdat, const char *dir, const char *repos,
...
*/
static int ls_dirleaveproc(void *callerdat, const char *dir, int err,
                            const char *update_dir, List *entries){
	if ( created_dir && !strcmp(created_dir, update_dir) ){
		...
		if ( unlink_file_dir(dir) ) 
			cvserr(0, errno, "Failed to remove directory `%s'",
			        created_dir);
