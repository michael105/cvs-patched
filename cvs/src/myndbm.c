...
/*  Returns NULL on error in which case errno has been set to indicate
    the error.  Can also callcvserr() itself.  */
/* ARGSUSED */
...
static int write_item(Node *node, void *data){
	FILE *fp = data;
	fputs(node->key, fp);
	fputs(" ", fp);
	fputs(node->data, fp);
...
void mydbm_close(DBM *db){
	if ( db->modified ){
		...
		if ( fp == NULL ) 
			cvserr(1, errno, "cannot write %s", db->name);
		walklist(db->dbm_list, write_item, fp);
		if ( fclose(fp) < 0 ) 
			cvserr(0, errno, "cannot close %s", db->name);
	}
	free(db->name);
	dellist(&db->dbm_list);
	free(db);
...
/*  Note: only updates the in-memory copy, which is written out at
    ...
    behavior.  */
int mydbm_store(DBM *db, datum key, datum value, int flags){
	Node *node;
	...
	*(char *)node->data = '\0';
	...
	if ( addnode(db->dbm_list, node) == -1 ){
		cvserr(0, 0, "attempt to insert duplicate key `%s'",
		        node->key);
		freenode(node);
		return 0;
	}
...
*/
static void mydbm_load_file(FILE *fp, List *list, char *filename){
	char *line = NULL;
	size_t line_size;
	char *value;
	...
	if ( line_length < 0 && !feof(fp) ) 
		cvserr(0, errno, "cannot read file `%s' in mydbm_load_file",
		        primary_root_inverse_translate(filename));
