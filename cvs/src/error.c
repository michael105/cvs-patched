...
/*  Print the program name and error message MESSAGE, which is a printf-style
    ...
    cvserr() does not molest errno; some code(e.g. Entries_Open) depends
...
void _cvserr(int status, int errnum, const char *message, ...){
	...
	*/
	...
	/*  Send the final message to the client or log it.
	    ...
	    here which can causecvserr() to be called a second time.
	*/
	...
	/*  Reset our recursion lock.  This needs to be done before the call to
	    exit() to allow the exit handlers to make calls tocvserr().
	*/
	last_message = NULL;
