...
int Create_Admin(const char *dir, const char *update_dir,
	...
	if ( isfile(tmp) ) 
		cvserr(1, 0, "there is a version in %s already", update_dir);
	...
	if ( CVS_MKDIR(tmp, 0777) < 0 ){
		...
		if ( warn ){
			...
			cvserr(0, errno, "warning: cannot make directory %s in %s",
			        CVSADM, update_dir);
			return 1;
		} else
			cvserr(1, errno, "cannot make directory %s in %s",
			        CVSADM, update_dir);
	} else{
		free(tmp);
		tmp = NULL;
	...
	if ( fout == NULL ){
		if ( update_dir[0] == '\0' ) 
			cvserr(1, errno, "cannot open %s", tmp);
		else
			cvserr(1, errno, "cannot open %s/%s", update_dir,
			        CVSADM_REP);
	}
	...
	*/
	...
	if ( fprintf(fout, "%s\n", cp) < 0 ){
		if ( update_dir[0] == '\0' ) 
			...
			cvserr(1, errno, "write to %s/%s failed", update_dir,
			        CVSADM_REP);
	}
	if ( fclose(fout) == EOF ){
		if ( update_dir[0] == '\0' ) 
			...
			cvserr(1, errno, "cannot close %s/%s", update_dir,
			        CVSADM_REP);
	}
