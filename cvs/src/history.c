...
/*  Get the name of the history log, either from CVSROOT/config, or via the
    hard-coded default.
*/
static const char *get_history_log_name(time_t now){
	char *log_name;
	...
	if ( config->HistoryLogPath ){
		...
		*/
		...
		if(!strftime (log_name, PATH_MAX, config->HistoryLogPath,
		               localtime(&now))){
			cvserr(0, 0, "Invalid date format in HistoryLogPath.");
	...
	while ( (c = getopt(argc, argv, "+Tacelow?D:b:f:m:n:p:r:t:u:x:X:z:")) != -1 ){
		...
		case 'D':			/* Since specified date */
			if ( *since_rev || *since_tag || *backto ){
				cvserr(0, 0, "date overriding rev/tag/backto");
		...
		case 'b':			/* Since specified file/Repos */
			if ( since_date || *since_rev || *since_tag ){
				cvserr(0, 0, "backto overriding date/rev/tag");
		...
		case 'r':			/* Since specified Tag/Rev */
			if ( since_date || *since_tag || *backto ){
				cvserr(0, 0, "rev overriding date/tag/backto");
				*since_tag = *backto = '\0';
				if ( since_date != NULL ) 
					free(since_date);
				since_date = NULL;
		...
		case 't':			/* Since specified Tag/Rev */
			if ( since_date || *since_rev || *backto ){
				cvserr(0, 0, "tag overriding date/marker/file/repos");
				*since_rev = *backto = '\0';
				if ( since_date != NULL ) 
					free(since_date);
				since_date = NULL;
			...
			extract++;
			{
				...
				for(cp = optarg; *cp; cp++)
					if ( !strchr(ALL_HISTORY_REC_TYPES, *cp) ) 
						cvserr(1, 0, "%c is not a valid report type", *cp);
			}
			free(rec_types);
				...
				*/
				...
				if ( get_date(&t, buf, NULL) ){
					...
					*/
					...
					cvserr(0, 0, "%s is not a known time zone", optarg);
				free(buf);
			}
			break;
		case '?':
...
*/
	...
	*/
	if ( lseek(fd, (off_t) 0, SEEK_END) == -1 ) 
		cvserr(1, errno, "cannot seek to end of history file: %s",
		        fname);

	if ( write(fd, line, strlen (line)) < 0 ) 
		cvserr(1, errno, "cannot write to history file: %s", fname);
	free(line);
	if ( close(fd) != 0 ) 
		cvserr(1, errno, "cannot close history file: %s", fname);
	free(workdir);
out:
	clear_history_lock();
}
...
static void save_user(char *name){
	if ( user_count == user_max ){
		...
		if ( size_overflow_p(xtimes (user_max, sizeof (char *))) ){
			cvserr(0, 0, "save_user: too many users");
			return;
		}
...
static void save_file(char *dir, char *name, char *module){
	...
	if ( file_count == file_max ){
		...
		if ( size_overflow_p(xtimes (file_max, sizeof (*fl))) ){
			cvserr(0, 0, "save_file: too many files");
			return;
		}
	...
	if ( dir && *dir ){
		...
		if ( name && *name ) 
			...
			cvserr(0, 0, "save_file: null dir and file name");
	}
}
...
static void save_module(char *module){
	if ( mod_count == mod_max ){
		...
		if ( size_overflow_p(xtimes (mod_max, sizeof (char *))) ){
			cvserr(0, 0, "save_module: too many modules");
			return;
		}
...
*/
static int read_hrecs_file(Node *p, void *closure){
	...
	if ( (fd = CVS_OPEN(fname, O_RDONLY | OPEN_BINARY)) < 0 ){
		cvserr(0, errno, "cannot open history file `%s'", fname);
		return 0;
	}

	if ( fstat(fd, &st_buf) < 0 ){
		cvserr(0, errno, "can't stat history file `%s'", fname);
		return 0;
	}

	if ( !(st_buf.st_size) ){
		cvserr(0, 0, "history file `%s' is empty", fname);
		return 0;
	}
	...
	for(;;){
		...
		if ( nl >= cpend ){
			if ( nl - cp >= STAT_BLOCKSIZE(st_buf) ){
				cvserr(1, 0, "history line %ld too long (> %lu)",
				        hrec_idx + 1,
			...
			if ( i < 0 ){
				cvserr(0, errno, "error reading history file `%s'", fname);
				return 0;
			}
			if ( nl == cp )  break;
			cvserr(0, 0,
...
static void read_hrecs(List *flist){
	...
	*/
	...
	if ( !files_read )
		cvserr(1, 0, "No history files read.");
...
static int select_hrec(struct hrec *hr){
	...
	if(!hr->type || !hr->user || !hr->dir || !hr->repos || !hr->rev ||
	        !hr->file || !hr->end){
		cvserr(0, 0, "warning: history line %ld invalid", hr->idx);
		return 0;
	}
...
static int accept_hrec(struct hrec *lr, struct hrec *hr){
	int ty;
