...
static void buf_default_memory_error (struct buffer *buf){
...
int buf_empty_p (struct buffer *buf){
...
*/
int buf_count_mem (struct buffer *buf){
	struct buffer_data *data;
	int mem = 0;
...
void buf_output (struct buffer *buf, const char *data, size_t len){
	if (buf->data != NULL
...
void buf_output0 (struct buffer *buf, const char *string){
...
void buf_append_char (struct buffer *buf, int ch){
	if (buf->data != NULL
	        && (buf->last->text + BUFFER_DATA_SIZE
	            != buf->last->bufp + buf->last->size)){
...
/*  Free struct buffer_data's from the list starting with FIRST and ending at
    LAST, inclusive.
*/
static inline void buf_free_datas (struct buffer_data *first,
...
*/
int buf_send_output (struct buffer *buf){
...
/*
    ...
    code.
*/
int buf_flush (struct buffer *buf, bool block){
	int nonblocking;
	int status;
...
/*
    ...
    code.
*/
int set_nonblock (struct buffer *buf){
	int status;
...
/*
    ...
    code.
*/
int set_block (struct buffer *buf){
	int status;
...
*/
int buf_send_counted (struct buffer *buf){
	int size;
	struct buffer_data *data;
...
/*
    ...
    within the same system.
*/
int buf_send_special_count (struct buffer *buf, int count){
	struct buffer_data *data;
...
void buf_append_data (struct buffer *buf, struct buffer_data *data,
                      struct buffer_data *last){
	if ( data != NULL ) {
		if ( buf->data == NULL ) 
			buf->data = data;
...
void buf_copy_data (struct buffer *buf, struct buffer_data *data,
	...
	while (1){
		new = get_buffer_data ();
		if ( !new )cvserr(1, errno, "Failed to allocate buffer data.");
...
void buf_free_data (struct buffer *buffer){
...
*/
void buf_append_buffer (struct buffer *to, struct buffer *from){
...
/*
    ...
    buf_append_data.
*/
int buf_read_file (FILE *f, long int size, struct buffer_data **retp,
...
*/
int buf_read_file_to_eof (FILE *f, struct buffer_data **retp,
                          struct buffer_data **lastp){
	int status;
...
int buf_chain_length (struct buffer_data *buf){
	int size = 0;
	while ( buf ) {
		size += buf->size;
		buf = buf->next;
...
/*
    ...
    bytes read.
*/
int buf_input_data (struct buffer *buf, size_t *countp){
...
*/
int buf_read_line (struct buffer *buf, char **line, size_t *lenp){
...
*/
int buf_read_data (struct buffer *buf, size_t want, char **retdata,
                   size_t *got){
	assert (buf->input != NULL);
...
*/
void buf_copy_lines (struct buffer *outbuf, struct buffer *inbuf, int command){
	while (1){
		struct buffer_data *data;
		struct buffer_data *nldata;
		char *nl;
...
/*
    ...
    copy something over.
*/
int buf_copy_counted (struct buffer *outbuf, struct buffer *inbuf,
                      int *special){
	*special = 0;
...
int buf_get_fd (struct buffer *buf){
	if ( buf->get_fd )
...
int buf_shutdown (struct buffer *buf){
...
static int packetizing_buffer_input (void *closure, char *data, size_t need,
                                     size_t size,
                                     size_t *got){
	...
	*got = 0;
	...
	while ( need > 0 || *got == 0 ) {
		...
		if ( tcount > count ) 
			cvserr(1, 0, "Input translation failure");
...
static int packetizing_buffer_output (void *closure, const char *data,
                                      size_t have,
                                      size_t *wrote){
	struct packetizing_buffer *pb = closure;
...
static int packetizing_buffer_flush (void *closure){
	struct packetizing_buffer *pb = closure;
...
static int packetizing_buffer_block (void *closure, bool block){
	struct packetizing_buffer *pb = closure;
...
static int packetizing_buffer_get_fd (void *closure){
...
static int packetizing_buffer_shutdown (struct buffer *buf){
...
*/
static int fd_buffer_input (void *closure, char *data, size_t need, size_t size,
                            size_t *got){
...
static int fd_buffer_output (void *closure, const char *data, size_t have,
                             size_t *wrote){
...
static int fd_buffer_block (void *closure, bool block){
	struct fd_buffer *fb = closure;
...
static int fd_buffer_get_fd (void *closure){
	struct fd_buffer *fb = closure;
	return fb->fd;
}
...
*/
static int fd_buffer_shutdown (struct buffer *buf){
...
# ifdef START_RSH_WITH_POPEN_RW
		...
		*/
			...
			cvserr(1, errno, "closing connection to %s",
			        fb->root->hostname);
			closefd = false;
		}
...
# ifdef CLIENT_SUPPORT
			...
			cvserr(1, errno, "closing down connection to %s",
			        fb->root->hostname);
		/* EXITS */
# endif /* CLIENT_SUPPORT */

		cvserr(0, errno, "closing down buffer");
	}
	...
	if ( fb->child_pid ){
		...
		if ( w == -1 ) 
			cvserr(1, errno, "waiting for process %d", fb->child_pid);
	}
