...
*/
int Parse_Info(const char *infofile, const char *repository, CALLPROC callproc,
	...
	if ( !current_parsed_root ){
		...
		cvserr(0, 0, "CVSROOT variable not set");
		return 1;
	}
	...
	if ( !fp_info ){
		...
		if ( !existence_error(errno) ) 
			cvserr(0, errno, "cannot open %s", infopath);
		free(infopath);
		return 0;
	}
	...
	while ( getline(&line, &line_allocated, fp_info) >= 0 ){
		...
		if ( *cp == '\0' ){
			cvserr(0, 0, "syntax error at line %d file %s; ignored",
		...
		*/
		...
		if ( regex_err ){
			cvserr(0, 0, "bad regular expression at line %d file %s: %s",
	...
	if ( ferror(fp_info) ) 
		cvserr(0, errno, "cannot read %s", infopath);
	if ( fclose(fp_info) < 0 ) 
...
*/
static bool readSizeT(const char *infopath, const char *option, const char *p,
                       size_t *val){
	const char *q;
	size_t num, factor = 1;
...
static inline struct config *new_config(void){
...
/*  Return true if this function has already been called for line LN of file
    INFOPATH.
*/
bool parse_error(const char *infopath, unsigned int ln){
 ...
 * *info files and various unix(or non-unix) config file syntaxes.
    ...
    ERRORS
     Callscvserr(0, ...) on errors in addition to the return value.

     xmalloc() failures are fatal, per usual.
*/
struct config* parse_config(const char *cvsroot, const char *path){
	const char *infopath;
	...
	*/
	...
	if ( path ){
		...
		if ( !approved )
			cvserr(1, 0, "Invalid path to config file specified: `%s'",
			        path);
		infopath = path;
		free(npath);
	} else
...
%s [%u]: warning: this CVS does not support PreservePermissions",
			...
			if ( retval->HistoryLogPath && !ISABSOLUTE(retval->HistoryLogPath) ){
				cvserr(0, 0, "%s [%u]: HistoryLogPath must be absolute.",
				        infopath, ln);
			...
			if(retval->HistorySearchPath
				...
				cvserr(0, 0, "%s [%u]: HistorySearchPath must be absolute.",
				        infopath, ln);
...
%s [%u]: warning: duplicate LogHistory entry found.",
...
#else /* !SUPPORT_OLD_INFO_FMT_STRINGS */
		{
			...
			/*  We may be dealing with a keyword which was added in a
			    ...
			    the new features.  */
			if ( !parse_error(infopath, ln) ) 
				cvserr(0, 0, "%s [%u]: unrecognized keyword `%s'",
				        infopath, ln, line);
	}
	if ( ferror(fp_info) ) 
		cvserr(0, errno, "cannot read %s", infopath);
