...
/*  This function has not been changed to deal with NO_SOCKET_TO_FD
    ...
    take care of this.  */
void start_kerberos4_server(cvsroot_t *root, struct buffer **to_server_p,
...

	{
		...
		if ( getsockname(s, (struct sockaddr *) &laddr, &laddrlen) < 0 ) 
			cvserr(1, 0, "getsockname failed: %s",
			        SOCK_STRERROR(SOCK_ERRNO));
		...
		if ( status != KSUCCESS ) 
			cvserr(1, 0, "kerberos authentication failed: %s",
...
void initialize_kerberos4_encryption_buffers( struct buffer **to_server_p,
        struct buffer **from_server_p ){
