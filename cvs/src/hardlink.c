...
static void delhardlist(Node *p){
	if ( p->data )
		dellist((List **)&p->data);
}
...
Node *lookup_file_by_inode(const char *filepath){
	char *inodestr;
	const char *file;
	struct stat sb;
	Node *hp, *p;
	...
	if ( stat(file, &sb) < 0 ){
		...
		cvserr(1, errno, "cannot stat %s", file);
	}
...
void update_hardlink_info(const char *file){
	char *path;
	Node *n;
	...
	if ( n == NULL ){
		...
		cvserr(1, 0, "lost hardlink info for %s", file);
	}
...
List *list_linked_files_on_disk(char *file){
	char *inodestr, *path;
	struct stat sb;
	Node *n;
...
int compare_linkage_lists(List *links1, List *links2){
	Node *n1, *n2;
	char *p1, *p2;
...
int find_checkedout_proc(Node *node, void *data){
	Node **uptodate = data;
	Node *link;
	char *dir = xgetcwd();
	char *path;
