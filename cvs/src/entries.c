...
*/
static Entnode *Entnode_Create(enum ent_type type, const char *user,
                                const char *vn,
...
static int write_ent_proc(Node *node, void *closure){
	...
	if ( fputentent(entfile, entnode) ) 
		cvserr(1, errno, "cannot write %s", entfilename);
...
static void write_entries(List *list){
	...
	if ( entfile == NULL ){
		...
		cvserr(0, errno, "cannot rewrite %s", entfilename);
	...
	if ( fclose(entfile) == EOF ) 
		cvserr(1, errno, "error closing %s", entfilename);
	...
	if ( unlink_file(CVSADM_ENTLOG )  < 0
		...
		cvserr(0, errno, "cannot remove %s", CVSADM_ENTLOG);
}
...
void Register(List *list, const char *fname, const char *vn, const char *ts,
	...
	if ( !noexec ){
		...
		if ( entfile == NULL ){
			...
			cvserr(0, errno, "cannot open %s", entfilename);
			return;
		}
...
static Entnode *fgetentent(FILE *fpin, char *cmd, int *sawdir){
	...
	if ( line_length < 0 && !feof(fpin) ) 
		cvserr(0, errno, "cannot read entries file");

	free(line);
	return ent;
}

static int fputentent(FILE *fp, Entnode *p){
	switch(p->type){
	case ENT_FILE:
		break;
	case ENT_SUBDIR:
...
List *Entries_Open(int aflag, char *update_dir){
	...
	*/
	...
	if ( fpin == NULL ){
		if ( update_dir != NULL ) 
			cvserr(0, 0, "in directory %s:", update_dir);
		cvserr(0, errno, "cannot open %s for reading", CVSADM_ENT);
	} else{
		...
		if ( fclose(fpin) < 0 ) 
			...
			cvserr(0, errno, "cannot close %s", CVSADM_ENT);
	}
	...
	if ( fpin != NULL ){
		...
		if ( fclose(fpin) < 0 ) 
			/* FIXME-update-dir: should include update_dir in message.  */
			cvserr(0, errno, "cannot close %s", CVSADM_ENTLOG);
	}
...
*/
void WriteTemplate(const char *update_dir, int xdotemplate,
...
void WriteTag(const char *dir, const char *tag, const char *date,
	...
	if ( tag || date ){
		...
		if ( tag ){
			if ( nonbranch ){
				if ( fprintf(fout, "N%s\n", tag) < 0 ) 
					cvserr(1, errno, "write to %s failed", tmp);
			} else{
				if ( fprintf(fout, "T%s\n", tag) < 0 ) 
					cvserr(1, errno, "write to %s failed", tmp);
			}
		...
		if ( fclose(fout) == EOF ) 
			cvserr(1, errno, "cannot close %s", tmp);
	} else if ( unlink_file(tmp) < 0 && ! existence_error (errno) ) 
		cvserr(1, errno, "cannot remove %s", tmp);
	free(tmp);
#ifdef SERVER_SUPPORT
	if ( server_active )
...
void ParseTag(char **tagp, char **datep, int *nonbranchp){
	...
	if ( fp ){
		...
		if ( line_length < 0 ){
			...
			if ( feof(fp) ) 
				cvserr(0, 0, "cannot read %s: end of file", CVSADM_TAG);
			else
				cvserr(0, errno, "cannot read %s", CVSADM_TAG);
		}

		if ( fclose(fp) < 0 ) 
			/* FIXME-update-dir: should include update_dir in message.  */
			cvserr(0, errno, "cannot close %s", CVSADM_TAG);
	...
	if ( sdtp != NULL && ! sdtp->subdirs ){
		...
		if ( !noexec ){
			...
			if ( fp == NULL ){
				...
				if ( ! isdir(CVSADM) ) 
					return;
				cvserr(1, save_errno, "cannot open %s", entfilename);
			} else{
				if ( fclose(fp) == EOF ) 
					cvserr(1, errno, "cannot close %s", entfilename);
			}
		}
	}
}
...
static void base_walk(enum base_walk code, struct file_info *finfo,
                       char **rev){
	FILE *fp;
	char *line;
	...
	if ( fp == NULL ){
		if ( !existence_error(errno) ){
			cvserr(0, errno, "cannot open %s for reading",
			        baserev_fullname);
			goto out;
		}
	}
	...
	case BASE_DEREGISTER:
		...
		if ( newf == NULL ){
			cvserr(0, errno, "cannot open %s for writing",
			        baserevtmp_fullname);
			goto out;
		}
		break;
	...
	if ( fp != NULL ){
		while ( getline(&line, &line_allocated, fp) >= 0 ){
			...
			if ( fncmp(linefile, finfo->file) == 0 ){
				...
				case BASE_DEREGISTER:
					if ( fprintf(newf, "%s\n", line) < 0 ) 
						cvserr(0, errno, "error writing %s",
						        baserevtmp_fullname);
					break;
				case BASE_GET:
					break;
		...
		if ( ferror(fp) ) 
			cvserr(0, errno, "cannot read %s", baserev_fullname);
	}
got_it:
...
out:
	...
	if ( fp != NULL ){
		if ( fclose(fp) < 0 ) 
			cvserr(0, errno, "cannot close %s", baserev_fullname);
	}
	if ( newf != NULL ){
		if ( fclose(newf) < 0 ) 
			cvserr(0, errno, "cannot close %s", baserevtmp_fullname);
...
char *base_get(struct file_info *finfo){
	char *rev;
...
void base_register(struct file_info *finfo, char *rev){
	base_walk(BASE_REGISTER, finfo, &rev);
}

/* Remove FILE.  */

void base_deregister(struct file_info *finfo){
