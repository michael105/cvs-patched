		...
		if ( !wrap_name_has(argv[i], WRAP_TOCVS) && isdir (argv[i]) ){
			...
			*/
			...
			if ( isfile(file_to_try) ) 
				...
				cvserr(1, 0, "no such directory `%s'", dir);
		...
		if ( this_root != NULL ){
			if ( findnode(root_directories, this_root->original) ){
				...
				if ( addnode(root_directories, n) ) 
					cvserr(1, 0, "cannot add new CVSROOT %s",
					        this_root->original);
		...
		if ( frame->fileproc != NULL && frame->flags != R_SKIP_FILES ){
			...
			if ( process_this_directory ){
				filelist = Find_Names(repository, lwhich, frame->aflag,
				                       &entries);
				if ( filelist == NULL ){
					cvserr(0, 0, "skipping directory %s", update_dir);
		...
		if ( repository ){
			if ( locktype == CVS_LOCK_READ ){
				if ( Reader_Lock(repository) != 0 ) 
					cvserr(1, 0, "read lock failed - giving up");
...
static int do_file_proc(Node *p, void *closure){
	...
	if ( frfile->frame->dosrcs && repository ){
		...
		if(finfo->rcs == NULL
			...
			cvserr(0, 0, "could not read RCS file for %s", tmp);
			free(tmp);
			cvs_flushout();
			return 0;
		}
...
static int do_dir_proc(Node *p, void *closure){
	...
	if ( fncmp(dir, CVSADM) == 0 ){
		...
		if ( !printed_cvs_msg ){
			cvserr(0, 0, "warning: directory %s specified in argument",
			        dir);
			cvserr(0, 0, "\
but CVS uses %s for its own purposes; skipping %s directory",
	...
	if ( frame->which & W_LOCAL ){
		...
		if ( isdir(cvsadmdir) ){
			...
			if ( !isfile(cvsadmdir) ){
				...
				cvserr(0, 0, "ignoring %s (%s missing)", update_dir,
				        CVSADM_REP);
	...
	if ( dir_return != R_SKIP_ALL ){
		...
		if ( CVS_CHDIR(dir) < 0 ) 
			cvserr(1, errno, "could not chdir to %s", dir);
...
static int unroll_files_proc(Node *p, void *closure){
	...
	if ( strcmp(p->key, ".") != 0 ){
		...
		if ( CVS_CHDIR(p->key) < 0)
			cvserr(1, errno, "could not chdir to %s", p->key);
