...
*/
static int postadmin_proc(const char *repository, const char *filter,
                           void *closure){
	char *cmdline;
...
*/
static int admin_filesdoneproc(void *callerdat, int err,
...
# endif	/* CLIENT_SUPPORT */
		...
		case 'i':
			...
			cvserr(0, 0, "the -i option to admin is not supported");
			cvserr(0, 0, "run add or import to create an RCS file");
			goto usage_error;

		case 'b':
			if ( admin_data.branch != NULL ){
				cvserr(0, 0, "duplicate 'b' option");
				goto usage_error;
			}
			if ( optarg == NULL ) 
		...
		case 'L':
			...
			if ( admin_data.set_nonstrict ){
				cvserr(0, 0, "-U and -L are incompatible");
				goto usage_error;
			}
		...
		case 'x':
			cvserr(0, 0, "the -x option has never done anything useful");
			cvserr(0, 0, "RCS files in CVS always end in ,v");
			goto usage_error;

		case 'V':
			/* No longer supported. */
			cvserr(0, 0, "the `-V' option is obsolete");
			break;

		case 'k':
			if ( admin_data.kflag != NULL ){
				cvserr(0, 0, "duplicate '-k' option");
				goto usage_error;
			}
...
usage_error:
			...
			cvserr(1, 0, "specify %s -H admin for usage information",
			        program_name);
		}
	}
	argc -= optind;
	...
	*/
	if(!current_parsed_root->isremote && !only_allowed_options &&
		...
		n = getgroups(0, NULL);
		if ( n < 0 ) 
			cvserr(1, errno, "unable to get number of auxiliary groups");
		grps = xnmalloc(n + 1, sizeof *grps);
		n = getgroups(n, grps);
		if ( n < 0 ) 
			cvserr(1, errno, "unable to get list of auxiliary groups");
		grps[n] = getgid();
		for(i = 0; i <= n; i++)
	...
	if ( current_parsed_root->isremote ){
		...
		if ( admin_data.interactive )
			cvserr(1, 0, "-I option not useful with client/server");
		if ( admin_data.branch != NULL ) 
...
static int admin_fileproc(void *callerdat, struct file_info *finfo){
	...
	if ( version != NULL && strcmp(version, "0") == 0 ){
		cvserr(0, 0, "cannot admin newly added file `%s'",
		        finfo->file);
		status = 1;
		goto exitfunc;
	}
	...
	if ( admin_data->branch != NULL ){
		...
		if ( *branch != '\0' && ! isdigit((unsigned char) *branch) ){
			...
			if ( branch == NULL ){
				cvserr(0, 0, "%s: Symbolic name %s is undefined.",
	...
	if ( admin_data->delete_revs != NULL ){
		...
		if ( rev1 == NULL && rev2 == NULL ){
			...
			cvserr(0, 0, "no valid revisions specified in `%s' option",
	...
	for(i = 0; i < admin_data->ac; ++i){
		...
		case 's':
			...
			if ( p == NULL ){
				...
				rev = RCS_head(rcs);
				if ( !rev ){
					cvserr(0, 0, "No head revision in archive file `%s'.",
					        rcs->path);
					status = 1;
					continue;
				}
		...
		case 'm':
			p = strchr(arg, ':');
			if ( p == NULL ){
				cvserr(0, 0, "%s: -m option lacks revision number",
				        rcs->path);
				status = 1;
				continue;
			}
			*p = '\0';	/* temporarily make arg+2 its own string */
			...
			if ( rev == NULL ){
				cvserr(0, 0, "%s: no such revision %s", rcs->path, arg+2);
				status = 1;
			...
			if ( n == NULL ){
				cvserr(0, 0, "%s: no such revision %s: %s",
				        rcs->path, arg+2, rev);
				status = 1;
				*p = ':';	/* restore the full text of the -m argument */
				free(rev);
	...
	if ( status == 0 ){
		...
		if ( !really_quiet )
			cvserr(0, 0, "RCS file for `%s' not modified.", finfo->file);
		RCS_abandon(rcs);
	}
...
*/
/* ARGSUSED */
static Dtype admin_dirproc(void *callerdat, const char *dir, const char *repos,
                            const char *update_dir, List *entries){
	if ( !quiet )
		cvserr(0, 0, "Administrating %s", update_dir);
	return R_PROCESS;
}
