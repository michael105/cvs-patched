...
int mkmodules(char *dir){
	...
	if ( CVS_CHDIR(dir) < 0 ) 
		cvserr(1, errno, "cannot chdir to %s", dir);
	...
	*/
	...
	if ( unlink_file(temp )  < 0
		...
		cvserr(0, errno, "cannot remove %s", temp);
	free(temp);
	...
	if ( fp ){
		...
		*/
		...
		if ( line )
			free(line);
		if ( ferror(fp) ) 
			cvserr(0, errno, "cannot read %s", CVSROOTADM_CHECKOUTLIST);
		if ( fclose(fp) < 0 ) 
			cvserr(0, errno, "cannot close %s", CVSROOTADM_CHECKOUTLIST);
	} else{
		/* Error from CVS_FOPEN.  */
		if ( !existence_error(errno) ) 
			cvserr(0, errno, "cannot open %s", CVSROOTADM_CHECKOUTLIST);
	}
...
static char *make_tempfile(void){
	...
	while(1){
		...
		if ( (fd = CVS_OPEN(temp, O_CREAT|O_EXCL|O_RDWR, 0666)) != -1 ) 
			break;
		if ( errno != EEXIST ) 
			cvserr(1, errno, "cannot create temporary file %s", temp);
	}
	if ( close(fd ) < 0)
		cvserr(1, errno, "cannot close temporary file %s", temp);
	return temp;
}
...
static int checkout_file(char *file, char *temp){
	...
	if ( !rcsnode ){
		/*  Probably not necessary(?); RCS_parsercsfile already printed a
		    message.  */
		cvserr(0, 0, "Failed to parse `%s'.", rcs);
		free(rcs);
		return 1;
	}
...
static void write_dbmfile( char *temp ){
	...
	if ( (db = dbm_open(temp, O_RDWR | O_CREAT | O_TRUNC, 0666)) == NULL ) 
		cvserr(1, errno, "cannot open dbm file %s for creation",
		        temp);
		...
		*/
		if ( !cont ){
			...
			*vp++ = '\0';		/* NULL terminate the key */
			...
			if ( *vp == '\0' ){
				cvserr(0, 0, "warning: NULL value for key `%s'", key.dptr);
				continue;
			}
			val.dptr = vp;
			val.dsize = strlen(vp);
			if ( dbm_store(db, key, val, DBM_INSERT) == 1 ){
				cvserr(0, 0, "duplicate key found for `%s'", key.dptr);
				err++;
			}
		}
	}
	dbm_close(db);
	if ( fclose(fp) < 0 ) 
		cvserr(0, errno, "cannot close %s", temp);
	if ( err ){
		...
		if ( unlink_file(dotdir )  < 0
		        && !existence_error(errno))
			cvserr(0, errno, "cannot remove %s", dotdir);
		if ( unlink_file(dotpag )  < 0
		        && !existence_error(errno))
			cvserr(0, errno, "cannot remove %s", dotpag);
		if ( unlink_file(dotdb )  < 0
		        && !existence_error(errno))
			cvserr(0, errno, "cannot remove %s", dotdb);
		cvserr(1, 0, "DBM creation failed; correct above errors");
	}
}
...
static void rename_dbmfile( char *temp ){
	...
	/*  I didn't want to callcvserr() when we had signals blocked
	    (unnecessary?), but do it now.  */
	if ( dir1_errno && !existence_error(dir1_errno) ) 
		cvserr(0, dir1_errno, "cannot remove %s", bakdir);
	if ( pag1_errno && !existence_error(pag1_errno) ) 
		cvserr(0, pag1_errno, "cannot remove %s", bakpag);
	if ( db1_errno && !existence_error(db1_errno) ) 
		cvserr(0, db1_errno, "cannot remove %s", bakdb);

	if ( dir2_errno && !existence_error(dir2_errno) ) 
		cvserr(0, dir2_errno, "cannot remove %s", bakdir);
	if ( pag2_errno && !existence_error(pag2_errno) ) 
		cvserr(0, pag2_errno, "cannot remove %s", bakpag);
	if ( db2_errno && !existence_error(db2_errno) ) 
		cvserr(0, db2_errno, "cannot remove %s", bakdb);

	if ( dir3_errno && !existence_error(dir3_errno) ) 
		cvserr(0, dir3_errno, "cannot remove %s", bakdir);
	if ( pag3_errno && !existence_error(pag3_errno) ) 
		cvserr(0, pag3_errno, "cannot remove %s", bakpag);
	if ( db3_errno && !existence_error(db3_errno) ) 
		cvserr(0, db3_errno, "cannot remove %s", bakdb);
}
...
static void rename_rcsfile(char *temp, char *real){
	...
	if ( stat(rcs, &statbuf )  < 0
	        && !existence_error(errno))
		cvserr(0, errno, "cannot stat %s", rcs);
	free(rcs);

	if ( chmod(temp, 0444 | (statbuf.st_mode & 0111)) < 0 ) 
		cvserr(0, errno, "warning: cannot chmod %s", temp);
	bak = Xasprintf("%s%s", BAKPREFIX, real);

	/* rm .#loginfo */
	if ( unlink_file(bak )  < 0
	        && !existence_error(errno))
		cvserr(0, errno, "cannot remove %s", bak);

	/* mv loginfo .#loginfo */
	if ( CVS_RENAME(real, bak )  < 0
	        && !existence_error(errno))
		cvserr(0, errno, "cannot rename %s to %s", real, bak);

	/* mv "temp" loginfo */
	if ( CVS_RENAME(temp, real )  < 0
	        && !existence_error(errno))
		cvserr(0, errno, "cannot rename %s to %s", temp, real);
...
int init(int argc, char **argv){
	...
	if ( CVS_CHDIR(adm) < 0)
		cvserr(1, errno, "cannot change to directory %s", adm);
		...
		if ( isfile(info_v) ) 
			...
			if ( !isfile(info) ){
				...
				for(p = fileptr->contents; *p != NULL; ++p)
					if ( fputs(*p, fp) < 0 ) 
						cvserr(1, errno, "cannot write %s", info);
				if ( fclose(fp) < 0 ) 
					cvserr(1, errno, "cannot close %s", info);
			}
