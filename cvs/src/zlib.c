...
static void compress_error(int status, int zstatus, z_stream *zstr,
                            const char *msg){
	int hold_errno;
	const char *zmsg;
	char buf[100];
...
static int compress_buffer_input(void *closure, char *data, size_t need,
                                  size_t size,
                                  size_t *got){
...
*/
static int compress_buffer_output(void *closure, const char *data, size_t have,
                                   size_t *wrote){
	struct compress_buffer *cb = closure;
...
static int compress_buffer_block(void *closure, bool block){
	struct compress_buffer *cb = closure;
...
static int compress_buffer_get_fd(void *closure){
	struct compress_buffer *cb = closure;
...
static int compress_buffer_shutdown_input(struct buffer *buf){
...
static int compress_buffer_shutdown_output(struct buffer *buf){
	struct compress_buffer *cb = buf->closure;
	int zstatus, status;
...
int gunzip_and_write(int fd, const char *fullname, unsigned char *buf,
                      size_t size){
	size_t pos;
	z_stream zstr;
	int zstatus;
	...
	if ( buf[3] & GZIP_FEXTRA ){
		if ( pos + 2 >= size ){
			cvserr(0, 0, "%s lacks proper gzip XLEN field", fullname);
			return 1;
		}
		pos += buf[pos] + (buf[pos + 1] << 8) + 2;
		if ( pos > size ){
			cvserr(0, 0, "%s lacks proper gzip \"extra field\"",
			        fullname);
			return 1;
		}
	...
	if ( buf[3] & GZIP_FNAME ){
		...
		if ( p == NULL ){
			cvserr(0, 0, "%s has bad gzip filename field", fullname);
			return 1;
		}
		pos = p - buf + 1;
	}
	if ( buf[3] & GZIP_FCOMMENT ){
		unsigned char *p = memchr(buf + pos, '\0', size - pos);
		if ( p == NULL ){
			cvserr(0, 0, "%s has bad gzip comment field", fullname);
			return 1;
		}
		pos = p - buf + 1;
	}
	if ( buf[3] & GZIP_FHCRC ){
		pos += 2;
		if ( pos > size ){
			cvserr(0, 0, "%s has bad gzip CRC16 field", fullname);
			return 1;
		}
	}
	...
	if ( pos >= size ){
		cvserr(0, 0, "gzip data incomplete for %s (no data)",
		        fullname);
		return 1;
	}
	...
	do{
		...
		if ( write(fd, outbuf, sizeof (outbuf) - zstr.avail_out) < 0 ){
			cvserr(0, errno, "writing decompressed file %s", fullname);
			return 1;
		}
	...
	pos += zstr.total_in;
	if ( size - pos != 8 ){
		cvserr(0, 0, "gzip data incomplete for %s (no trailer)",
		        fullname);
		return 1;
	}
	...
	if ( crc != ((unsigned long ) buf[pos]
		...
		cvserr(0, 0, "CRC error uncompressing %s", fullname);
		return 1;
	}
	...
	if ( zstr.total_out != ((unsigned long ) buf[pos + 4]
		...
		cvserr(0, 0, "invalid length uncompressing %s", fullname);
		return 1;
	}
...
int read_and_gzip(int fd, const char *fullname, unsigned char **buf,
                   size_t *size,
	...
	while(1){
		...
		if ( nread < 0 ){
			cvserr(0, errno, "cannot read %s", fullname);
			return 1;
		} else if ( nread == 0 ) 
			/* End of file.  */
			finish = 1;
