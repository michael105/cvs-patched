...
static void fileattr_delproc(Node *node){
...
char *fileattr_get(const char *filename, const char *attrname){
	Node *node;
...
char *fileattr_get0 (const char *filename, const char *attrname){
	char *cp;
	char *cpend;
	char *retval;
...
char *fileattr_modify(char *list, const char *attrname, const char *attrval,
                       int namevalsep, int entsep){
	char *retval;
	char *rp;
	size_t attrname_len = strlen(attrname);
...
void fileattr_set(const char *filename, const char *attrname,
                   const char *attrval){
	Node *node;
	char *p;
...
char *fileattr_getall(const char *filename){
	Node *node;
	char *p;
...
void fileattr_setall(const char *filename, const char *attrs){
	Node *node;
...
static int writeattr_proc(Node *node, void *data){
	FILE *fp = (FILE *)data;
	fputs("F", fp);
	fputs(node->key, fp);
	fputs("\t", fp);
...
*/
static int postwatch_proc(const char *repository, const char *filter,
                           void *closure){
	char *cmdline;
	...
	*/
	...
	if ( !cmdline || !strlen(cmdline) ){
		...
		cvserr(0, 0, "postwatch proc resolved to the empty string!");
		return 1;
	}
...
void fileattr_write(void){
	...
	if ( list_isempty(attrlist ) 
		...
		if ( unlink_file(fname) < 0 ){
			if ( !existence_error(errno) ){
				cvserr(0, errno, "cannot remove %s", fname);
			}
		}
	...
	if ( fp == NULL ){
		if ( existence_error(errno) ){
			...
			if ( CVS_MKDIR(repname, 0777) < 0 && errno != EEXIST ){
				cvserr(0, errno, "cannot make directory %s", repname);
				(void) umask(omask);
				free(fname);
				free(repname);
				return;
		...
		if ( fp == NULL ){
			cvserr(0, errno, "cannot write %s", fname);
			(void) umask(omask);
			free(fname);
			return;
		}
