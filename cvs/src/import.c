...
int import(int argc, char **argv){
	...
	while ( (c = getopt(argc, argv, "+Qqdb:m:I:k:W:X")) != -1 ){
		...
		case 'd':
			if ( server_active ){
				...
				cvserr(0, 0, "due to client limitations");
			}
			use_file_modtime = 1;
			break;
		case 'b':
	...
	/*  Don't allow CVSDIRNAME as any directory in module path.

	    Could abstract this to valid_module_path, but I don't think we'll need
	    to call it from anywhere else.
	*/
	if ( (cp = strstr(argv[0], CVSDIRNAME) ) 
	        &&   /* path contains CVSDIRNAME AND ... */
		...
		cvserr(1, 0, "as a directory in a path or as a file name.");
	}
	...
	for(i = 1; i < argc; i++){	/* check the tags for validity */
		...
		for(j = 1; j < i; j++)
			if ( strcmp(argv[j], argv[i]) == 0 ) 
				cvserr(1, 0, "tag `%s' was specified more than once",
				        argv[i]);
	}
	...
	if ( current_parsed_root == NULL ){
		cvserr(0, 0, "missing CVSROOT environment variable\n");
		cvserr(1, 0, "Set it or specify the '-d' option to %s.",
		        program_name);
	}
	...
	*cp = '\0';
	...
	if ( !safe_location(NULL) ){
		cvserr(1, 0, "attempt to import the repository");
	}
	...
	*/
	...
	if ( (logfp = cvs_temp_file(&tmpfile)) == NULL ) 
		cvserr(1, errno, "cannot create temporary file `%s'",
		        tmpfile);
	/*  On systems where we can unlink an open file, do so, so it will go
	    ...
	    errors but I'm not sure whichcvserr(s) we get if we are on a system
	...
	*/
	...
	dellist(&ulist);
	if ( fclose(logfp) < 0 ) 
		cvserr(0, errno, "error closing %s", tmpfile);
...
/*  Process all the files in ".", then descend into other directories.
    Returns 0 for success, or >0 oncvserr(in which case a message
...
static int import_descend(char *message, char *vtag, int targc, char **targv){
	...
	if ( (dirp = CVS_OPENDIR(".")) == NULL ){
		cvserr(0, errno, "cannot open directory");
		err++;
	} else{
		errno = 0;
...
one_more_time_boys:
		...
		if ( errno != 0 ){
			cvserr(0, errno, "cannot read directory");
			++err;
		}
		...
		if ( !isfile(attic_name) ){
			...
			if ( !killnew )
				...
				if ( CVS_MKDIR(rcs, 0777 ) != 0 && errno != EEXIST ) 
					cvserr(1, errno, "cannot make directory `%s'", rcs);
		...
		*/
		...
		if ( tocvsPath )
			if ( unlink_file_dir(tocvsPath) < 0 ) 
				cvserr(0, errno, "cannot remove %s", tocvsPath);
...
/*  Create a new RCS file from scratch.
    ...
    INPUTS
     ...
     add_logfp  Write errors to here as well as viacvserr(), or NULL if we
                should use onlycvserr().
...
*/
int add_rcs_file(const char *message, const char *rcs, const char *user,
	...
	if ( do_killnew ){
		...
		if ( ++last_number <= 0 ) 
			cvserr(1, 0, "invalid revision number %s", add_vhead);
	...
	if ( lstat(userfile, &sb) < 0 ){
		...
		cvserr(0, errno, "cannot lstat file %s", userfile);
		goto read_error;
	}
	...
	if (
		...
		if ( fpuser == NULL ){
			...
			cvserr(0, errno, "ERROR: cannot read file %s", userfile);
			goto read_error;
		}
	}
	...
	*/
	...
	if ( fpuser != NULL ){
		if ( fclose(fpuser) < 0 ) 
			cvserr(0, errno, "cannot close %s", user);
	}
	...
	*/
	...
	if ( chmod(rcs, mode) < 0 ){
		...
		cvserr(0, ierrno, "WARNING: cannot change mode of file %s",
		        rcs);
		err++;
	}
	if ( tocvsPath )
...
const char *user;
{
	if (
		...
		while(1){
			...
			if ( len == 0 ){
				if ( ferror(fpuser) ) 
					cvserr(1, errno, "cannot read file %s for copying",
					        user);
				break;
			}
...
*/
int expand_at_signs(const char *buf, size_t size, FILE *fp){
...
*/
static int import_descend_dir(char *message, char *dir, char *vtag, int targc,
                               char **targv){
	...
	if ( !quiet && !current_parsed_root->isremote ) 
		cvserr(0, 0, "Importing %s", repository);

	if ( CVS_CHDIR(dir) < 0 ){
		ierrno = errno;
		fperrmsg(logfp, 0, ierrno, "ERROR: cannot chdir to %s", repository);
		cvserr(0, ierrno, "ERROR: cannot chdir to %s", repository);
		err = 1;
		goto out;
	}
	if ( !current_parsed_root->isremote && !isdir(repository) ){
		...
		if ( isfile(repository) || isfile (rcs) ){
			fperrmsg(logfp, 0, 0,
			          ...
			          repository);
			cvserr(0, 0, "ERROR: %s is a file, should be a directory!",
			        repository);
			err = 1;
			goto out;
		}
