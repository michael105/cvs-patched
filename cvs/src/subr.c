...
char *Xreadlink(const char *link, size_t size){
	...
	if ( file == NULL ) 
		cvserr(1, errno, "cannot readlink %s", link);
...
void xrealloc_and_strcat(char **str, size_t *lenp, const char *src){
	bool newstr = !*lenp;
...
/*  Return the number of levels that PATH ascends above where it starts.
    For example:
     ...
     "foo/../../bar" -> 1
*/
int pathname_levels(const char *p){
	int level;
	int max_level;
...
# ifndef __GNUC__
/* ARGSUSED */
bool get_date(struct timespec *result, char const *p,
...
char *gca(const char *rev1, const char *rev2){
	...
	if ( rev1 == NULL || rev2 == NULL ){
		cvserr(0, 0, "sanity failure in gca");
		abort();
	}
	...
	*--g = '\0';
	...
	if ( dots == 0 ){
		...
		if ( *p1 == '\0' ){
			...
			cvserr(0, 0, "bad revisions %s or %s", rev1, rev2);
			abort();
		} else{
...
void check_numeric(const char *rev, int argc, char **argv){
	...
	if(argc != 1
		...
		cvserr(0, 0, "while processing more than one file:");
		cvserr(1, 0, "attempt to specify a numeric revision");
	}
}
...
int file_has_markers(const struct file_info *finfo){
	...
	if ( fp == NULL ) 
		cvserr(1, errno, "cannot open %s", finfo->fullname);
	...
	if ( ferror(fp) ) 
		cvserr(0, errno, "cannot read %s", finfo->fullname);
out:
	if ( fclose(fp) < 0 ) 
		cvserr(0, errno, "cannot close %s", finfo->fullname);
	if ( line != NULL ) 
		free(line);
	return result;
}
...
void get_file(const char *name, const char *fullname, const char *mode,
	...
	if ( name == NULL ){
		...
		if ( stat(name, &s) < 0 ) 
			cvserr(1, errno, "can't stat %s", fullname);
	...
	while(1){
		...
		if ( ferror(e) ) 
			cvserr(1, errno, "can't read %s", fullname);
		nread += got;
		tobuf += got;
	...
	if ( e != stdin && fclose(e) < 0 ) 
		cvserr(0, errno, "cannot close %s", fullname);
...
#else
		cvserr(1, 0, "internal error: islink doesn't like readlink");
#endif
	}
}
	...
	while ( (pfmt = va_arg(args, char *)) != NULL ){
		...
		*buf = '%';
		...
		if ( conversion_error || s[1] ){
			cvserr(1, 0,
			        "internalcvserr(format_cmdline): '%s' is not a valid conversion!!!",
			        conversion);
		}
	...
	*/
	...
	if ( inquotes ){
		/*  FIXME - we shouldn't need this - Parse_Info should be handling
		    multiple lines...
		*/
		cvserr(1, 0, "unterminated quote in format string: %s",
		        format);
	}
...
/*  Print a warning and return false if P doesn't look like a string specifying
    ...
    RETURNS
     ...
     false	On error.
*/
bool readBool(const char *infopath, const char *option, const char *p,
               bool *val){
	...
	cvserr(0, 0, "%s: unrecognized value `%s' for `%s'",
	        infopath, p, option);
	return false;
}
...
FILE *xfopen(const char *name, const char *mode){
	...
	if ( !(fp = fopen(name, mode)) ) 
		cvserr(1, errno, "cannot open %s", name);
	return fp;
}
...
char *xcanonicalize_file_name(const char *path){
	...
	if ( !hardpath )
		cvserr(1, errno, "Failed to resolve path: `%s'", path);
	return hardpath;
}
...
bool isThisHost(const char *otherhost){
	...
	*/
	...
	if ( !fqdno )
		cvserr(1, 0, "Name lookup failed for `%s': %s",
