...
*/
void do_editor(const char *dir, char **messagep, const char *repository,
                List *changes){
	...
	if ( strcmp(Editor, "") == 0 ) 
		cvserr(1, 0, "no editor defined, must use -e or -m");

again:
	/* Create a temporary file.  */
	if( ( fp = cvs_temp_file( &fname ) ) == NULL )
		cvserr( 1, errno, "cannot create temporary file" );
	...
	if ( repository != NULL ) 
		...
		if ( tfp == NULL ){
			if ( !existence_error(errno) ) 
				cvserr(1, errno, "cannot read %s", CVSADM_TEMPLATE);
		} else{
			while ( !feof(tfp) ){
				char *p = buf;
			...
			if ( fclose(tfp) < 0 ) 
				cvserr(0, errno, "cannot close %s", CVSADM_TEMPLATE);
		}
	}
	...
	if ( stat(fname, &post_stbuf) != 0 ) 
		cvserr(1, errno, "cannot find size of temp file %s", fname);
	...
	if ( *messagep ){
		...
		while(1){
			...
			if ( line_length == -1 ){
				if ( ferror(fp) ) 
					cvserr(0, errno, "warning: cannot read %s", fname);
				break;
			}
	...
	if ( fclose(fp) < 0 ) 
		cvserr(0, errno, "warning: cannot close %s", fname);
	...
	if ( pre_stbuf.st_mtime == post_stbuf.st_mtime || *messagep == NULL ){
		for(;;){
			...
			if ( line_length < 0 ){
				cvserr(0, errno, "cannot read from stdin");
			...
			if ( *line == 'a' || *line == 'A' ){
				if ( unlink_file(fname) < 0 ) 
					cvserr(0, errno, "warning: cannot remove temp file %s",
					        fname);
				cvserr(1, 0, "aborted by user");
			}
			if ( *line == 'e' || *line == 'E' ) 
				goto again;
			if ( *line == '!' ){
...
void do_verify(char **messagep, const char *repository, List *changes){
	...
	if((err = Parse_Info (CVSROOTADM_VERIFYMSG, repository,
		...
		/* Since followingcvserr() exits, delete the temp file now.  */
		if ( data.fname != NULL && unlink_file( data.fname ) < 0 ) 
			cvserr(0, errno, "cannot remove %s", data.fname);
		free(data.fname);

		errno = saved_errno;
		cvserr(1, err == -1 ? errno : 0,
	...
	*/
	if(config->RereadLogAfterVerify == LOGMSG_REREAD_ALWAYS ||
		...
		if(stat(data.fname, &post_stbuf) != 0)
			cvserr(1, errno, "cannot find size of temp file %s",
			        data.fname);
	}
	...
	if(config->RereadLogAfterVerify == LOGMSG_REREAD_ALWAYS ||
		...
		if ( post_stbuf.st_size == 0 ) 
			...
			*messagep[0] = '\0';
				...
				if ( line_length == -1 ){
					if ( ferror(fp) ) 
						/*  Fail in this case because otherwise we will have no
						    log message
						*/
						cvserr(1, errno, "cannot read %s", data.fname);
					break;
				}
				if ( strncmp(line, CVSEDITPREFIX, CVSEDITPREFIXLEN) == 0 ) 
					continue;
			...
			if ( line ) free(line);
			if ( fclose(fp) < 0 ) 
				cvserr(0, errno, "warning: cannot close %s", data.fname);
		}
	}
...
static int rcsinfo_proc(const char *repository, const char *template,
	...
	if ( (tfp = CVS_FOPEN(template, "r")) != NULL ){
		...
		while ( getline(&line, &line_chars_allocated, tfp) >= 0 ) 
			(void) fputs(line, fp);
		if ( ferror(tfp) ) 
			cvserr(0, errno, "warning: cannot read %s", template);
		if ( fclose(tfp) < 0 ) 
			cvserr(0, errno, "warning: cannot close %s", template);
		if ( line )
			free(line);
		return(0);
	} else{
...
void Update_Logfile(const char *repository, const char *xmessage, FILE *xlogfp,
                     List *xchanges){
	struct ulp_data ud;
...
static int logfile_write(const char *repository, const char *filter,
	...
	*/
	...
	if ( !cmdline || !strlen(cmdline) ){
		...
		cvserr(0, 0, "logmsg proc resolved to the empty string!");
		return 1;
	}

	if ( (pipefp = run_popen(cmdline, "w")) == NULL ){
		if ( !noexec )
			cvserr(0, 0, "cannot write entry to log filter: %s",
			        cmdline);
		free(cmdline);
		return 1;
	}
...
static int verifymsg_proc(const char *repository, const char *script,
	...
	if ( vpd->fname == NULL ){
		...
		if ( config->RereadLogAfterVerify == LOGMSG_REREAD_STAT ){
			...
			if ( stat(vpd->fname, &(vpd->pre_stbuf)) != 0 ) 
				cvserr(1, errno, "cannot stat temp file %s", vpd->fname);
	...
	*/
	...
	if ( !verifymsg_script || !strlen(verifymsg_script) ){
		...
		cvserr(0, 0, "verifymsg proc resolved to the empty string!");
		return 1;
	}
