 ...
 *.so *.Z *~ *.old *.elc *.ln *.bak *.BAK *.orig *.rej *.exe _$* *$ .svn .git";
...
void ign_add_file(char *file, int hold){
	...
	if ( fp == NULL ){
		if ( ! existence_error(errno) ) 
			cvserr(0, errno, "cannot open %s", file);
		return;
	}
	while ( getline(&line, &line_allocated, fp) >= 0 ) 
		ign_add(line, hold);
	if ( ferror(fp) ) 
		cvserr(0, errno, "cannot read %s", file);
	if ( fclose(fp) < 0 ) 
		...
		if ( (!*(ign+1 ) || isspace((unsigned char) *(ign+1)))
			...
			if ( !hold ){
				...
				for(i = 0; i < ign_count; i++)
					free(ign_list[i]);
				ign_count = 1;
				/* Always ignore the CVSDIRNAME directory.  */
				ign_list[0] = xstrdup(CVSDIRNAME);
				ign_list[1] = NULL;
...
*/
void ignore_files(List *ilist, List *entries, const char *update_dir,
                   Ignore_proc proc){
	int subdirs;
	DIR *dirp;
	struct dirent *dp;
	...
	dirp = CVS_OPENDIR(".");
	if ( dirp == NULL ){
		cvserr(0, errno, "cannot open current directory");
		return;
	}
	...
	if ( errno != 0 ) 
		cvserr(0, errno, "error reading current directory");
