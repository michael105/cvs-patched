...
*/
int change_mode(const char *filename, const char *mode_string,
                 int respect_umask){
#ifdef CHMOD_BROKEN
	char *p;
	int writeable = 0;
...
*/
static size_t read_line_via(struct buffer *via_from_buffer,
	...
	if ( status != 0 ) 
		cvserr(1, status, "writing to server");
	...
	if ( status != 0 ){
		if ( status == -1 ) 
			...
			cvserr(1, 0, "out of memory");
		else
			cvserr(1, status, "reading from server");
	}
...
static size_t read_line(char **resultp){
...
static void handle_ok(char *args, size_t len){
	return;
}



static void handle_error(char *args, size_t len){
	...
	*/
	char *p = strchr(args, ' ');
	if ( !p ){
		cvserr(0, 0, "invalid data from cvs server");
		return;
	}
	++p;
...
static void handle_valid_requests(char *args, size_t len){
	char *p = args;
	char *q;
	struct request *rq;
	do{
	...
	for(rq = requests; rq->name; ++rq){
		...
		if ( rq->flags & RQ_ESSENTIAL ) 
			cvserr(1, 0, "request `%s' not supported by server",
			        rq->name);
	}
}

static void handle_force_gzip(char *args, size_t len){
	force_gzip = true;
}
...
static void handle_referrer(char *args, size_t len){
...
*/
static void handle_redirect(char *args, size_t len){
...
static void call_in_directory(const char *pathname,
	...
	*/
	...
	if ( !toplevel_wd ){
		toplevel_wd = xgetcwd();
		if ( !toplevel_wd )
			cvserr(1, errno, "could not get working directory");
	}

	if ( CVS_CHDIR(toplevel_wd) < 0 ) 
		cvserr(1, errno, "could not chdir to %s", toplevel_wd);
	...
	if ( CVS_CHDIR(dir_name) < 0 ){
		...
		if ( ! existence_error(errno) ) 
			cvserr(1, errno, "could not chdir to %s", dir_name);
		...
		do{
			...
			if ( fncmp(dir, CVSADM) == 0 ){
				cvserr(0, 0, "cannot create a directory named %s", dir);
				cvserr(0, 0, "because CVS uses \"%s\" for its own uses",
				        CVSADM);
				cvserr(1, 0, "rename the directory and try again");
			}
...
static void copy_a_file(void *data, List *ent_list, const char *short_pathname,
                         const char *filename){
	char *newname;
	...
	if ( unlink_file(newname) && !existence_error (errno) ) 
		cvserr(0, errno, "unable to remove %s", newname);
...
static void handle_copy_file(char *args, size_t len){
...
protocol error: compressed files not supported for that operation");
	...
	if ( !fp )
		cvserr(1, errno, "cannot write %s", fullname);
	nread = size;
	nwrite = 0;
	pread = buf;
	pwrite = buf;
	...
	free(buf);
	if ( fclose(fp) < 0 ) 
		cvserr(1, errno, "cannot close %s", fullname);
}
...
static void handle_checksum(char *args, size_t len){
	char *s;
	char buf[3];
	int i;

	if ( stored_checksum_valid )
		cvserr(1, 0, "Checksum received before last one was used");
	...
	if ( i < 16 || *s != '\0' ) 
		cvserr(1, 0, "Invalid Checksum response: `%s'", args);
...
static void handle_mode(char *args, size_t len){
	if ( stored_mode )
		cvserr(1, 0, "protocol error: duplicate Mode");
...
static void handle_mod_time(char *args, size_t len){
	struct timespec newtime;
	if ( stored_modtime_valid )
		cvserr(0, 0, "protocol error: duplicate Mod-time");
	if ( get_date(&newtime, args, NULL) ){
		/* Truncate nanoseconds.  */
		stored_modtime = newtime.tv_sec;
		stored_modtime_valid = 1;
	} else
		cvserr(0, 0, "protocol error: cannot parse date %s", args);
}
...
static void update_entries(void *data_arg, List *ent_list,
	...
	*/
	...
	if ( scratch_entries[0] != '/' ) 
		cvserr(1, 0, "bad entries line `%s' from server",
		        entries_line);
	...
	*cp++ = '\0';
	...
	if(data->contents == UPDATE_ENTRIES_UPDATE
		...
		if(data->existp == UPDATE_ENTRIES_EXISTING
			...
			cvserr(0, 0, "warning: %s unexpectedly disappeared",
			        short_pathname);
		...
		if(data->existp == UPDATE_ENTRIES_NEW
			...
			cvserr(0, 0, "move away `%s'; it is in the way",
			        short_pathname);
			if ( updated_fname ){
				cvs_output("C ", 0);
...
discard_file_and_return:
		...
		if ( data->contents == UPDATE_ENTRIES_RCS_DIFF ){
			...
			if ( fd < 0 ){
				/*  I can see a case for making this a fatal error; for
				    ...
				    the same problem.  */
				cvserr(0, errno, "cannot write %s", short_pathname);
				free(temp_filename);
				free(buf);
			...
			if ( size > 0 ){
				...
				if ( use_gzip ){
					if(gunzip_and_write (fd, short_pathname,
						...
						cvserr(1, 0, "aborting due to compression error");
				} else if ( write(fd, buf, size) != size ) 
					cvserr(1, errno, "writing %s", short_pathname);
			}
		...
		if ( data->contents == UPDATE_ENTRIES_UPDATE ){
			...
			if ( !isfile(filename) ) 
				cvserr(1, 0, "patch original file %s does not exist",
				        short_pathname);
			filebuf = NULL;
			filebufsize = 0;
			nread = 0;
			...
			if(! rcs_change_text (short_pathname, filebuf, nread, buf, size,
				...
				if ( ! patch_failed ){
					...
					if ( fwrite(patchedbuf, sizeof *patchedbuf, patchedlen, e ) 
					        != patchedlen)
						cvserr(1, errno, "cannot write %s", temp_filename);
					if ( fclose(e) == EOF ) 
						cvserr(1, errno, "cannot close %s", temp_filename);
					rename_file(temp_filename, filename);
				}
		...
		if ( stored_checksum_valid && ! patch_failed ){
			...
			*/
			...
			if ( !e )
				cvserr(1, errno, "could not open %s", short_pathname);

			md5_init_ctx(&context);
			while ( (len = fread(buf, 1, sizeof buf, e)) != 0 ) 
				md5_process_bytes(buf, len, &context);
			if ( ferror(e) ) 
				cvserr(1, errno, "could not read %s", short_pathname);
			...
			if ( memcmp(checksum, stored_checksum, 16) != 0 ){
				if ( data->contents != UPDATE_ENTRIES_PATCH ) 
					cvserr(1, 0, "checksum failure on %s",
					        short_pathname);
...

		{
			...
			if ( status != 0 ) 
				cvserr(0, status, "cannot change mode of %s",
				        short_pathname);
		}
	...
	if ( stored_modtime_valid ){
		...
		if ( utime(filename, &t) < 0 ) 
			cvserr(0, errno, "cannot set time on %s", filename);
...
static void handle_checked_in(char *args, size_t len){
...
static void handle_new_entry(char *args, size_t len){
	struct update_entries_data dat;
	dat.contents = UPDATE_ENTRIES_CHECKIN;
	dat.existp = UPDATE_ENTRIES_EXISTING_OR_NEW;
...
static void handle_updated(char *args, size_t len){
	struct update_entries_data dat;
...
static void handle_created(char *args, size_t len){
	struct update_entries_data dat;
	dat.contents = UPDATE_ENTRIES_UPDATE;
...
static void handle_update_existing(char *args, size_t len){
	struct update_entries_data dat;
	dat.contents = UPDATE_ENTRIES_UPDATE;
...
static void handle_merged(char *args, size_t len){
	struct update_entries_data dat;
	dat.contents = UPDATE_ENTRIES_UPDATE;
...
static void handle_patched(char *args, size_t len){
	struct update_entries_data dat;
...
static void handle_rcs_diff(char *args, size_t len){
	struct update_entries_data dat;
...
static void remove_entry(void *data, List *ent_list,
                          const char *short_pathname,
                          const char *filename){
...
static void handle_remove_entry(char *args, size_t len){
	call_in_directory(args, remove_entry, NULL);
}



static void remove_entry_and_file(void *data, List *ent_list,
                                   const char *short_pathname,
                                   const char *filename){
	Scratch_Entry(ent_list, filename);
	...
	if ( unlink_file(filename) < 0 ) 
		cvserr(0, errno, "unable to remove %s", short_pathname);
}



static void handle_removed(char *args, size_t len){
...
static void set_static(void *data, List *ent_list, const char *short_pathname,
                        const char *filename){
	FILE *fp;
	fp = xfopen(CVSADM_ENTSTAT, "w+");
	if ( fclose(fp) == EOF ) 
		cvserr(1, errno, "cannot close %s", CVSADM_ENTSTAT);
}



static void handle_set_static_directory(char *args, size_t len){
	if ( !strcmp(cvs_cmd_name, "export") ){
...
static void clear_static(void *data, List *ent_list,
                          const char *short_pathname,
                          const char *filename){
	if ( unlink_file(CVSADM_ENTSTAT) < 0 && ! existence_error (errno) ) 
		cvserr(1, errno, "cannot remove file %s", CVSADM_ENTSTAT);
}



static void handle_clear_static_directory(char *pathname, size_t len){
	if ( !strcmp(cvs_cmd_name, "export") ){
		/* Swallow the repository.  */
		read_line(NULL);
		return;
...
static void set_sticky(void *data, List *ent_list, const char *short_pathname,
                        const char *filename){
	char *tagspec;
	FILE *f;
	...
	if ( !f ){
		...
		cvserr(0, errno, "cannot open %s", CVSADM_TAG);
		free(tagspec);
		return;
	}
	if ( fprintf(f, "%s\n", tagspec) < 0 ) 
		cvserr(1, errno, "writing %s", CVSADM_TAG);
	if ( fclose(f) == EOF ) 
		cvserr(1, errno, "closing %s", CVSADM_TAG);
	free(tagspec);
}



static void handle_set_sticky(char *pathname, size_t len){
	if ( !strcmp(cvs_cmd_name, "export") ){
		/* Swallow the repository.  */
		read_line(NULL);
...
static void clear_sticky(void *data, List *ent_list,
                          const char *short_pathname,
                          const char *filename){
	if ( unlink_file(CVSADM_TAG) < 0 && ! existence_error (errno) ) 
		cvserr(1, errno, "cannot remove %s", CVSADM_TAG);
}



static void handle_clear_sticky(char *pathname, size_t len){
	if ( !strcmp(cvs_cmd_name, "export") ){
		/* Swallow the repository.  */
		read_line(NULL);
		return;
...
*/
static void handle_edit_file(char *pathname, size_t len){
	call_in_directory(pathname, edit_file, NULL);
}



static void template(void *data, List *ent_list, const char *short_pathname,
                      const char *filename){
...
static void handle_template(char *pathname, size_t len){
	call_in_directory(pathname, template, NULL);
}



static void clear_template(void *data, List *ent_list,
                            const char *short_pathname,
                            const char *filename){
	if ( unlink_file(CVSADM_TEMPLATE) < 0 && ! existence_error (errno) ) 
		cvserr(1, errno, "cannot remove %s", CVSADM_TEMPLATE);
}



static void handle_clear_template(char *pathname, size_t len){
	...
	for(p = prune_candidates; p; ){
		if ( isemptydir(p->dir, 1) ){
			...
			if ( unlink_file_dir(p->dir) < 0 ) 
				cvserr(0, errno, "cannot remove %s", p->dir);
			b = strrchr(p->dir, '/');
			if ( !b )
...
static void send_repository(const char *dir, const char *repos,
	...
	*/
	...
	if ( update_dir && *update_dir != '\0' && strcmp(update_dir, "." ) 
		...
		if ( addnode(dirs_sent_to_server, n) ) 
			cvserr(1, 0, "cannot add directory %s to list", n->key);
	}
	...
	if ( supported_request("Sticky") ){
		...
		if ( !f ){
			if ( ! existence_error(errno) ) 
				cvserr(1, errno, "reading %s", adm_name);
		} else{
			char line[80];
			char *nl = NULL;
			...
			if ( !nl )
				send_to_server("\012", 1);
			if ( fclose(f) == EOF ) 
				cvserr(0, errno, "closing %s", adm_name);
		}
	}
	free(adm_name);
...
void send_a_repository(const char *dir, const char *repository,
...
static void notified_a_file(void *data, List *ent_list,
                             const char *short_pathname,
                             const char *filename){
	FILE *fp;
	FILE *newf;
	...
	if ( getline(&line, &line_len, fp) < 0 ){
		if ( feof(fp) ) 
			cvserr(0, 0, "cannot read %s: end of file", CVSADM_NOTIFY);
		else
			cvserr(0, errno, "cannot read %s", CVSADM_NOTIFY);
		goto error_exit;
	}
	cp = strchr(line, '\t');
	if ( !cp ){
		cvserr(0, 0, "malformed %s file", CVSADM_NOTIFY);
		goto error_exit;
	}
	*cp = '\0';
	if ( strcmp(filename, line + 1) ) 
		cvserr(0, 0, "protocol error: notified %s, expected %s",
		        filename,
		        line + 1);
	...
	if ( getline(&line, &line_len, fp) < 0 ){
		if ( feof(fp) ){
			free(line);
			if ( fclose(fp) < 0 ) 
				cvserr(0, errno, "cannot close %s", CVSADM_NOTIFY);
			if ( CVS_UNLINK(CVSADM_NOTIFY) < 0)
				cvserr(0, errno, "cannot remove %s", CVSADM_NOTIFY);
			return;
		} else{
			cvserr(0, errno, "cannot read %s", CVSADM_NOTIFY);
			goto error_exit;
	...
	if ( fputs(line, newf) < 0 ){
		cvserr(0, errno, "cannot write %s", CVSADM_NOTIFYTMP);
		goto error2;
	}
	...
	if ( fclose(newf) < 0 ){
		cvserr(0, errno, "cannot close %s", CVSADM_NOTIFYTMP);
		goto error_exit;
	}
	free(line);
	if ( fclose(fp) < 0 ){
...
static void handle_notified(char *args, size_t len){
...
static void handle_module_expansion(char *args, size_t len){
	if ( !modules_vector ){
...
void client_expand_modules(int argc, char **argv, int local){
	...
	if ( errs )
		cvserr(errs, 0, "cannot expand modules");
}
...
static void handle_wrapper_rcs_option(char *args, size_t len){
	char *p;
...
static void handle_m(char *args, size_t len){
...
static void handle_mbinary(char *args, size_t len){
	char *size_string;
	size_t size;
	size_t totalread;
	size_t nread;
...
static void handle_e(char *args, size_t len){
	/*  In the case where stdout and stderr point to the same place,
...
/*ARGSUSED*/
static void handle_f  (char *args, size_t len){
	fflush(stderr);
}



static void handle_mt(char *args, size_t len){
	char *p;
	char *tag = args;
	char *text;
...
/*
    ...
    contain 0's.
*/
void send_to_server_via(struct buffer *via_buffer, const char *str,
                         size_t len){
	static int nbytes;
	...
	if ( nbytes >= 2 * BUFFER_DATA_SIZE ){
		...
		if ( status != 0 ) 
			cvserr(1, status, "error writing to server");
		nbytes = 0;
	}
}
...
static size_t try_read_from_server( char *buf, size_t len ){
	int status;
	size_t nread;
	char *data;
...
static inline void close_connection_to_server(struct buffer **to,
        struct buffer **from){
	int status;
	...
	*/
	...
	if ( status != 0 ) 
		cvserr(0, status, "shutting down buffer to server");
	buf_free(*to);
	*to = NULL;

	status = buf_shutdown(*from);
	if ( status != 0 ) 
		cvserr(0, status, "shutting down buffer from server");
	buf_free(*from);
	*from = NULL;
}
...
bool supported_request(const char *name){
	struct request *rq;
...
/*  Generic function to do port number lookup tasks.
    ...
    In order of precedence, will return:
  	...
  	defaultport
*/
static int get_port_number(const char *envname, const char *portname,
                            int defaultport){
	struct servent *s;
	char *port_s;
	...
	if ( envname && (port_s = getenv(envname)) ){
		...
		if ( port <= 0 ){
			cvserr(0, 0, "%s must be a positive integer!  If you",
			        envname);
			cvserr(0, 0,
...
/*  get the port number for a client to connect to based on the proxy port
    of a cvsroot_t.
*/
static int get_proxy_port_number(const cvsroot_t *root){
...
void make_bufs_from_fds(int tofd, int fromfd, int child_pid, cvsroot_t *root,
...
# endif /* NO_SOCKET_TO_FD */
	{
		...
		if ( tofd == fromfd ){
			fromfd = dup(tofd);
			if ( fromfd < 0 ) 
				cvserr(1, errno, "cannot dup net connection");
		}
...
/*  Connect to the authenticating server.
    ...
    error.  */
void connect_to_pserver(cvsroot_t *root, struct buffer **to_server_p,
	...
	if ( sock == -1 ) 
		cvserr(1, 0, "cannot create socket: %s",
	...
	if ( connect(sock, &client_sai.addr, sizeof (client_sai) ) 
	        < 0)
		cvserr(1, 0, "connect to %s(%s):%d failed: %s",
...
static void auth_server(cvsroot_t *root, struct buffer *to_server,
...
*/
static void connect_to_forked_server(cvsroot_t *root,
                                      struct buffer **to_server_p,
...
# else /* SERVER_SUPPORT */
	{
		cvserr( 0, 0,
		        "You must set the CVS_SERVER environment variable when" );
		cvserr( 0, 0, "using the :fork: access method." );
		cvserr( 1, 0,
		        "This CVS was not compiled with server support." );
	}
# endif /* SERVER_SUPPORT */
	...
	if ( child_pid < 0 ) 
		cvserr(1, 0, "could not fork server process");
...
static int send_variable_proc(Node *node, void *closure){
...
/*  Open up the connection to the server and perform any necessary
    authentication.
*/
void open_connection_to_server(cvsroot_t *root, struct buffer **to_server_p,
                                struct buffer **from_server_p){
...
#else /* AUTH_CLIENT_SUPPORT */
		cvserr(0, 0,
		        "CVSROOT is set for a pserver access method but your");
		cvserr(1, 0, "CVS executable doesn't support it.");
#endif /* AUTH_CLIENT_SUPPORT */
		break;
...
#ifdef NO_EXT_METHOD
		cvserr(0, 0,
		        ":ext: method not supported by this port of CVS");
		cvserr(1, 0, "try :server: instead");
#else /* ! NO_EXT_METHOD */
...
#  ifdef HAVE_KERBEROS
			if ( current_parsed_root->method == kserver_method ){
				if ( !supported_request("Kerberos-encrypt") ) 
					cvserr(1, 0, "This server does not support encryption");
...
#else /* ! ENCRYPTION */
			cvserr(1, 0, "This client does not support encryption");
#endif /* ! ENCRYPTION */
		}
	...
	*/
	...
	if ( !strcmp (cvs_cmd_name, "import") || !strcmp (cvs_cmd_name, "add") ){
		if ( supported_request("wrapper-sendme-rcsOptions") ){
			...
			if ( err != 0 ) 
				cvserr(err, 0, "error reading from server");
		}
	}
...
#else /* ! HAVE_GSSAPI */
		cvserr(1, 0,
		        "This client does not support stream authentication");
#endif /* ! HAVE_GSSAPI */
	}
...
static void send_modified(const char *file, const char *short_pathname,
                           Vers_TS *vers){
	...
	if ( stat(file, &sb) < 0 ) 
		cvserr(1, errno, "reading %s", short_pathname);
	...
	if ( file_gzip_level && sb.st_size > 100 ){
		...
		if ( close(fd) < 0 ) 
			cvserr(0, errno, "warning: can't close %s", short_pathname);
		...
		if ( !bin ){
			...
			if ( CVS_UNLINK(tfile) < 0 ) 
				cvserr(0, errno, "warning: can't remove temp file %s",
				        tfile);
			free(tfile);
		}
#endif
...
static void send_ignproc(const char *file, const char *dir){
...
static int send_filesdoneproc(void *callerdat, int err, const char *repository,
...
*/
static Dtype send_dirent_proc(void *callerdat, const char *dir,
	...
	if ( ignore_directory(update_dir) ){
		...
		if ( !quiet )
			cvserr(0, 0, "Ignoring %s", update_dir);
		return R_SKIP_ALL;
	}
...
*/
/* ARGSUSED */
static int send_dirleave_proc(void *callerdat, const char *dir, int err,
                               const char *update_dir, List *entries ){
...
void send_options(int argc, char * const *argv){
	int i;
	for(i = 0; i < argc; i++)
		send_arg(argv[i]);
}
...
*/
int client_process_import_file(char *message, char *vfile, char *vtag,
                                int targc,
	...
	if ( modtime ){
		if ( supported_request("Checkin-time") ){
			...
			if ( stat(vfile, &sb) < 0 ) 
				cvserr(1, errno, "cannot stat %s", fullname);
...
void client_notify(const char *repository, const char *update_dir,
