...
DBM *open_module(void){
	...
	if ( current_parsed_root == NULL ){
		cvserr(0, 0, "must set the CVSROOT environment variable");
		cvserr(1, 0, "or specify the '-d' global option");
	}
...
*/
int my_module(DBM *db, char *mname, enum mtype m_type, char *msg,
	...
	*/
	if ( ISABSOLUTE(mname) ) 
		cvserr(1, 0, "Absolute module reference invalid: `%s'",
		        mname);
	...
	*/
	...
	cvserr(0, 0, "cannot find module `%s' - ignored", mname);
	err++;
	goto do_module_return;
...
found:
	...
	*/
	...
	if ( modargc == 0  &&  spec_opt == NULL ){
		cvserr(0, 0, "modules file missing directory for module %s",
		        mname);
		++err;
		goto do_module_return;
	}
...
do_special:
	...
	while ( spec_opt != NULL ){
		...
		if ( *spec_opt == '\0' ) 
			cvserr(0, 0, "Mal-formed %c option for module %s - ignored",
...
void cat_module(int status){
	...
	if ( !(db = open_module()) ) 
		cvserr(1, 0, "failed to open the modules file");
