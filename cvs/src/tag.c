...
int cvstag(int argc, char **argv){
	...
	if ( date && delete_flag ) 
		cvserr(1, 0, "-d makes no sense with a date specification.");
	if ( delete_flag && branch_mode ) 
		cvserr(0, 0, "warning: -b ignored with -d options");
...
*/
static int posttag_proc(const char *repository, const char *filter,
                         void *closure){
	char *cmdline;
	...
	*/
	...
	if ( !cmdline || !strlen(cmdline) ){
		...
		cvserr(0, 0, "pretag proc resolved to the empty string!");
		return 1;
	}
...
*/
static int tag_filesdoneproc(void *callerdat, int err, const char *repository,
...
*/
/* ARGSUSED */
static int rtag_proc(int argc, char **argv, char *xwhere, char *mwhere,
                      char *mfile,
	...
	if ( is_rtag ){
		...
		*/
		...
		if ( CVS_CHDIR(repository) < 0 ){
			cvserr(0, errno, "cannot chdir to %s", repository);
			free(repository);
			free(where);
			return 1;
		}
	...
	if ( err ){
		cvserr(1, 0, "correct the above errors first!");
	}
...
static int check_fileproc(void *callerdat, struct file_info *finfo){
	...
	if ( check_uptodate ){
		...
		case T_REMOVED:
		default:
			cvserr(0, 0, "%s is locally modified", finfo->fullname);
			freevers_ts(&vers);
			return 1;
		}
	} else
	...
	if ( vers->srcfile == NULL ){
		if ( !really_quiet )
			cvserr(0, 0, "nothing known about %s", finfo->file);
		freevers_ts(&vers);
		freenode(p);
		return 1;
	}
...
static int check_filesdoneproc(void *callerdat, int err, const char *repos,
	...
	if((n = Parse_Info (CVSROOTADM_TAGINFO, repos, pretag_proc, PIOPT_ALL,
	                     &ppd)) > 0){
		cvserr(0, 0, "Pre-tag check failed");
		err += n;
	}
	return err;
}
...
static int pretag_proc(const char *repository, const char *filter,
	...
	*/
	...
	/*  FIXME - the old code used to run the following here:
	    ...
	    if ( !isfile(s) )
	   {
	      cvserr(0, errno, "cannot find pre-tag filter '%s'", s);
	       free(s);
	       return(1);
	    }
...
*/
static int pretag_list_to_args_proc(Node *p, void *closure){
	...
	*/
	...
	if ( version == NULL ){
		...
		if ( !quiet && !force_tag_match ){
			cvserr(0, 0, "cannot find tag `%s' in `%s'",
			...
			*/
			...
			if ( !force_tag_move ){
				...
				if ( (isbranch && !disturb_branch_tags )  ||
					...
					cvserr(0,0, "%s: Not moving %s tag `%s' from %s to %s%s.",
					        finfo->fullname,
...
static int tag_fileproc(void *callerdat, struct file_info *finfo){
	...
	*/
	...
	if ( !version )
		...
		if ( !quiet )
			cvserr(0, 0, "couldn't tag added but un-commited file `%s'",
			        finfo->file);
		goto free_vars_and_return;
	} else if ( version[0] == '-' ){
		if ( !quiet )
			cvserr(0, 0, "skipping removed but un-commited file `%s'",
			        finfo->file);
		goto free_vars_and_return;
	} else if ( vers->srcfile == NULL ){
		if ( !quiet )
			cvserr(0, 0, "cannot find revision control file for `%s'",
			        finfo->file);
		goto free_vars_and_return;
	}
...
*/
/* ARGSUSED */
static Dtype tag_dirproc(void *callerdat, const char *dir, const char *repos,
                          const char *update_dir, List *entries){
	...
	if ( !quiet )
		cvserr(0, 0, "%s %s", delete_flag ? "Untagging" : "Tagging",
		        update_dir);
	return R_PROCESS;
}
	...
	*/
	...
	if ( idb ){
		...
		if ( !db ){
			...
			cvserr(0, errno, "warning: cannot open `%s' read/write",
			        valtags_filename);
			*idb = NULL;
			return 1;
		}
		...
		*idb = db;
		...
		if ( !db && !existence_error(errno) ) 
			cvserr(1, errno, "cannot read %s", valtags_filename);
	}
...
static void add_to_val_tags(const char *name){
	...
	*/
	...
	if ( dbm_store(db, mytag, value, DBM_REPLACE) < 0 ) 
		cvserr(0, errno, "failed to store %s into val-tags", name);
	dbm_close(db);
...
static Dtype val_direntproc(void *callerdat, const char *dir,
...
/*  With VALID set, insert NAME into val-tags if it is not already present
    ...
    RETURNS
     Nothing.
*/
void tag_check_valid(const char *name, int argc, char **argv, int local,
                      int aflag,
	...
	if ( !valid ){
		...
		*/
		...
		if ( repository == NULL || repository[0] == '\0' ) 
			...
			if ( CVS_CHDIR(repository) < 0 ) 
				cvserr(1, errno, "cannot change to %s directory",
				        repository);
		}
