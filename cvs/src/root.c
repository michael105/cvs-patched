...
cvsroot_t *Name_Root(const char *dir, const char *update_dir){
	...
	*/
	...
	if ( (len = getline(&root, &root_allocated, fpin)) < 0 ){
		...
		cvserr(0, 0, "in directory %s:", xupdate_dir);
		cvserr(0, saved_errno, "cannot read %s", CVSADM_ROOT);
		cvserr(0, 0, "please correct this problem");
		ret = NULL;
		goto out;
	}
	fclose(fpin);
...
static void delconfig(Node *n){
	if ( n->data ) free_config(n->data);
}



void root_allow_add(const char *arg, const char *configPath){
	Node *n;
...
bool root_allow_ok(const char *arg){
	if ( !root_allow ){
...
*/
static void free_cvsroot_t(cvsroot_t *root){
	assert(root);
...
/*
    ...
    SEE ALSO
  	free_cvsroot_t()
*/
cvsroot_t *parse_cvsroot(const char *root_in){
...
*/
	...
	*/
	...
	if ( *cvsroot_copy == ':' ){
		...
		*/
		...
		if ( ! (p = strchr(method, ':')) ){
			cvserr(0, 0, "No closing `:' on method in CVSROOT.");
			goto error_exit;
		}
		*p = '\0';
		cvsroot_copy = ++p;
		...
		*/
		...
		if ( !strcasecmp(method, "local") ) 
			...
			cvserr(0, 0, "Unknown method (`%s') in CVSROOT.", method);
			goto error_exit;
		}
		...
		while ( (p = strtok(NULL, ";")) ){
			char *q = strchr(p, '=');
			if ( q == NULL ){
				cvserr(0, 0, "Option (`%s') has no argument in CVSROOT.",
				        p);
				goto error_exit;
			}
			...
			*q++ = '\0';
			...
			if ( !strcasecmp(p, "proxy") ){
				...
				cvserr(0, 0, "Unknown option (`%s') in CVSROOT.", p);
				goto error_exit;
			}
		}
#endif /* defined(CLIENT_SUPPORT) || defined (SERVER_SUPPORT) */
	...
	if ( (newroot->method != local_method ) 
		...
		*/
		if ( (p = strchr(cvsroot_copy, '/')) == NULL ){
			cvserr(0, 0, "CVSROOT requires a path spec:");
			cvserr(0, 0,
			        ":(gserver|kserver|pserver):[[user][:password]@]host[:[port]]/path");
			cvserr(0, 0, "[:(ext|server):][[user]@]host[:]/path");
			goto error_exit;
		}
			...
			if ( strlen(p) ){
				...
				if ( (newroot->port = atoi(p)) <= 0 ){
					cvserr(0, 0,
					        "CVSROOT may only specify a positive, non-zero, integer port (not `%s').",
					        p);
					cvserr(0, 0, "Perhaps you entered a relative pathname?");
					goto error_exit;
				}
			}
		}
	...
	*/
	...
	if ( newroot->username && ! newroot->hostname ){
		cvserr(0, 0, "Missing hostname in CVSROOT.");
		goto error_exit;
	}
	...
	*/
	...
	case local_method:
		...
		if ( newroot->username || newroot->hostname ){
			...
			cvserr(0, 0, "when using local access method.");
			goto error_exit;
		}
#endif /* defined(CLIENT_SUPPORT) || defined(SERVER_SUPPORT) */
	...
	case fork_method:
		...
		if ( newroot->username || newroot->hostname ){
			cvserr(0, 0,
			        "Can't specify hostname and username in CVSROOT");
			cvserr(0, 0, "when using fork access method.");
			goto error_exit;
		}
...
#endif /* defined(CLIENT_SUPPORT) || defined(SERVER_SUPPORT) */
	default:
		cvserr(1, 0, "Invalid method found in parse_cvsroot");
	}
	...
	if ( check_hostname && !newroot->hostname ){
		cvserr(0, 0, "Didn't specify hostname in CVSROOT.");
		goto error_exit;
	}
	...
	if ( no_port && newroot->port ){
		...
		cvserr(0, 0, "and pserver connection methods.");
		goto error_exit;
	}
#endif /* defined(CLIENT_SUPPORT) || defined(SERVER_SUPPORT) */

	if ( *newroot->directory == '\0' ){
		cvserr(0, 0, "Missing directory in CVSROOT.");
		goto error_exit;
	}
...
static int get_local_root_dir(Node *p, void *root_in){
