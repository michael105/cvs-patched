...
static void find_ignproc(const char *file, const char *dir){
	struct question *p;
...
static int find_filesdoneproc(void *callerdat, int err, const char *repository,
                               const char *update_dir, List *entries){
	struct find_data *find_data = callerdat;
...
static int find_fileproc(void *callerdat, struct file_info *finfo){
	...
	if ( vers->vn_user == NULL ){
		if ( vers->ts_user == NULL ) 
			cvserr(0, 0, "nothing known about `%s'", finfo->fullname);
		else
			cvserr(0, 0, "use `%s add' to create an entry for `%s'",
	...
	if ( vers->vn_user[0] == '-' ){
		...
		if ( vers->ts_user == NULL ){
			...
			if ( !really_quiet )
				cvserr(0, 0, "warning: new-born %s has disappeared",
				        finfo->fullname);
	...
	*/
	...
	if ( geteuid() == (uid_t) 0 && !current_parsed_root->isremote ){
		...
		if ( pw->pw_uid == (uid_t) 0 ) 
			cvserr(1, 0, "'root' is not allowed to commit files");
	}
#endif /* CVS_BADROOT */
	...
	if ( logfile ){
		...
		if ( saved_message )
			cvserr(1, 0, "cannot specify both a message and a log file");
	...
	if ( current_parsed_root->isremote ){
		...
		err = start_recursion
		      (find_fileproc, find_filesdoneproc, find_dirent_proc, NULL,
		       ...
		       NULL, 0, NULL );
		if ( err )
			cvserr(1, 0, "correct above errors first!");
		...
		*/
		...
		if ( err != 0 && use_editor && saved_message != NULL ){
			...
			if ( fp == NULL ) 
				cvserr(1, 0, "cannot create temporary file %s", fname);
			if ( fwrite(saved_message, 1, strlen (saved_message), fp ) 
			        != strlen(saved_message))
				cvserr(1, errno, "cannot write temporary file %s", fname);
			if ( fclose(fp) < 0 ) 
				cvserr(0, errno, "cannot close temporary file %s", fname);
			cvserr(0, 0, "saving log message in %s", fname);
			free(fname);
		}
		return err;
	}
...
static
Ctype classify_file_internal(struct file_info *finfo, Vers_TS **vers){
...
static int check_fileproc(void *callerdat, struct file_info *finfo){
	...
	if(strncmp (finfo->repository, current_parsed_root->directory,
		...
		cvserr(1, 0, "cannot check in to %s", finfo->repository);
	...
	*/
	...
	case T_NEEDS_MERGE:
	case T_REMOVE_ENTRY:
		cvserr(0, 0, "Up-to-date check failed for `%s'",
		        finfo->fullname);
		goto out;
	case T_CONFLICT:
	case T_MODIFIED:
...
warning: file `%s' seems to still contain conflict indicators",
		...
		if ( check_valid_edit && editor == NULL ){
			cvserr(0, 0, "Valid edit does not exist for %s",
			        finfo->fullname);
			freevers_ts(&vers);
			return 1;
		}
	...
	case T_UPTODATE:
		break;
	default:
		cvserr(0, 0, "CVS internal error: unknown status %d",
		        status);
		break;
	}
...
static int precommit_proc(const char *repository, const char *filter,
	...
	*/
	...
	if ( !cmdline || !strlen(cmdline) ){
		...
		cvserr(0, 0, "precommit proc resolved to the empty string!");
		return 1;
	}
...
static int check_filesdoneproc(void *callerdat, int err, const char *repos,
	...
	n = Parse_Info(CVSROOTADM_COMMITINFO, repos, precommit_proc, PIOPT_ALL,
	                saved_ulist);
	if ( n > 0 ){
		cvserr(0, 0, "Pre-commit check failed");
		err += n;
	}
...
static int commit_fileproc(void *callerdat, struct file_info *finfo){
	...
	*/
	...
	if ( ci->status == T_MODIFIED ){
		if ( finfo->rcs == NULL ) 
			cvserr(1, 0, "internal error: no parsed RCS file");
...
/*
    ...
    Return value is 0 on success, or >0 oncvserr(in which case we have
...
static int remove_file(struct file_info *finfo, char *tag, char *message){
	...
	if ( branch ){
		...
		if ( rev == NULL ){
			cvserr(0, 0, "cannot find branch \"%s\".", tag);
			return 1;
		}
	...
	if ( !tag && !branch ){
		if ( RCS_setbranch(finfo->rcs, NULL) != 0 ){
			cvserr(0, 0, "cannot change branch to default for %s",
			        finfo->fullname);
			return 1;
		}
		RCS_rewrite(finfo->rcs, NULL, NULL);
...
*/
static int finaladd(struct file_info *finfo, char *rev, char *tag,
                     char *options){
	int ret;
...
static void unlockrcs(RCSNode *rcs){
	...
	if ( (retcode = RCS_unlock(rcs, NULL, 1)) != 0 ) 
		cvserr(retcode == -1 ? 1 : 0, retcode == -1 ? errno : 0,
...
static void fixaddfile(const char *rcs){
	...
	if ( (rcsfile = RCS_parsercsfile(rcs)) == NULL ){
		if ( unlink_file(rcs) < 0 ) 
			cvserr(0, errno, "cannot remove %s", rcs);
	} else
		freercsnode(&rcsfile);
	really_quiet = save_really_quiet;
}
...
static int checkaddfile(const char *file, const char *repository,
	...
	if ( *rcsnode == NULL ){
		...
		if ( adding_on_branch ){
			...
			if ( CVS_MKDIR(rcsname, 0777) != 0 && errno != EEXIST ) 
				cvserr(1, errno, "cannot make directory `%s'", rcsname);
			(void) umask(omask);
		...
		*rcsnode = rcs;
		...
		if((oldexpand != NULL
			...
			/*  We tell the user about this, because it means that the
			    ...
			    used to.  */
			cvserr(0, 0, "changing keyword expansion mode to %s",
			        options);
		...
		if ( !adding_on_branch ){
			...
			if ( !(rcs->flags & INATTIC) ){
				cvserr(0, 0, "warning: expected %s to be in Attic",
				        rcs->path);
			}
		...
		if ( lock_RCS(file, rcs, rev, repository) ){
			cvserr(0, 0, "cannot lock revision %s in `%s'.",
	...
	if ( adding_on_branch ){
		if ( newfile ){
			...
			fp = fopen(file, "w");
			if ( fp == NULL ) 
				cvserr(1, errno, "cannot open %s for writing", file);
			if ( fclose(fp) < 0 ) 
				cvserr(0, errno, "cannot close %s", file);
			...
			if ( rcs == NULL ){
				cvserr(0, 0, "could not read %s", rcs->path);
				goto out;
			}
			*rcsnode = rcs;
			...
			if ( lock_RCS(file, rcs, NULL, repository) ){
				cvserr(0, 0, "cannot lock initial revision in `%s'.",
				        rcs->path);
				goto out;
			}
		}
		...
		if ( !RCS_nodeisbranch(rcs, tag) ){
			...
			if ( !head )
				cvserr(1, 0, "No head revision in archive file `%s'.",
				        rcs->print_path);
			...
			if ( lock_RCS(file, rcs, NULL, repository) ){
				cvserr(0, 0, "cannot lock head revision in `%s'.",
				        rcs->path);
				goto out;
			}
		}
