...
int cvslog(int argc, char **argv){
	...
	if ( current_parsed_root->isremote ){
		...
		if ( is_rlog && !supported_request("rlog") ) 
			cvserr(1, 0, "server does not support rlog");
...
static int rlog_proc(int argc, char **argv, char *xwhere, char *mwhere,
                      char *mfile,
...
static int log_fileproc(void *callerdat, struct file_info *finfo){
	...
	if ( rcsfile == NULL ){
		...
		if ( baserev != NULL ){
			if ( baserev[0] == '0' && baserev[1] == '\0' ){
				if ( !really_quiet )
					cvserr(0, 0, "%s has been added, but not committed",
					        finfo->file);
				return 0;
			}
		}
	...
	if ( !log_data->header && ! log_data->long_header && rcsfile->head != NULL ){
		...
		if ( p == NULL ) 
			cvserr(1, 0, "can not find head revision in `%s'",
			        finfo->fullname);
		while ( p != NULL ){
			RCSVers *vers = p->data;
			...
			if ( vers->next == NULL ) 
				...
				if ( p == NULL ) 
					cvserr(1, 0, "can not find next revision `%s' in `%s'",
...
*/
static struct revlist *log_expand_revlist(RCSNode *rcs, char *baserev,
	...
	for(r = revlist; r != NULL; r = r->next){
		...
		if ( r->first == NULL && r->last == NULL ){
			...
			if ( !nr->first ){
				if ( !really_quiet )
					cvserr(0, 0, "No head revision in archive `%s'.",
					        rcs->path);
				nr->last = NULL;
				nr->fields = 0;
			} else{
			...
			if ( !nr->first ){
				if ( !really_quiet )
					cvserr(0, 0, "warning: no branch `%s' in `%s'",
			...
			if ( r->first == NULL || isdigit((unsigned char) r->first[0]) ) 
				...
				if ( nr->first == NULL && !really_quiet ){
					cvserr(0, 0, "warning: no revision `%s' in `%s'",
					        r->first, rcs->print_path);
				}
			}
...
/*
    ...
    options provided.
*/
static int log_version_requested(struct log_data *log_data,
                                  struct revlist *revlist,
...
static int log_fix_singledate(Node *p, void *closure){
	...
	if ( pv == NULL ) 
		cvserr(1, 0, "missing version `%s' in RCS file `%s'",
...
*/
static void log_tree(struct log_data *log_data, struct revlist *revlist,
                      RCSNode *rcs,
                      const char *ver){
	Node *p;
	RCSVers *vnode;
...
*/
static void log_abranch(struct log_data *log_data, struct revlist *revlist,
                         RCSNode *rcs,
                         const char *ver){
	Node *p;
	RCSVers *vnode;
...
*/
static void log_version(struct log_data *log_data, struct revlist *revlist,
                         RCSNode *rcs,
...
static Dtype log_dirproc(void *callerdat, const char *dir,
	...
	if ( !quiet )
		cvserr(0, 0, "Logging %s", update_dir);
	return R_PROCESS;
}
