...
static void call_diff_setup(const char *prog, int argc, char * const *argv){
	int i;
...
static void call_diff_error(const char *format, const char *a1,
                             const char *a2){
	/*  FIXME: Should we somehow indicate that this error is coming from
	    the diff library?  */
	cvserr(0, 0, format, a1, a2);
}
...
int RCS_merge(RCSNode *rcs, const char *path, const char *workfile,
	...
	/* Clean up. */
	{
		...
		if ( unlink_file(tmp1) < 0 ){
			if ( !existence_error(errno) ) 
				cvserr(0, errno, "cannot remove temp file %s", tmp1);
		}
		free(tmp1);
		if ( unlink_file(tmp2) < 0 ){
			if ( !existence_error(errno) ) 
				cvserr(0, errno, "cannot remove temp file %s", tmp2);
		}
		free(tmp2);
		if ( diffout ){
			if ( unlink_file(diffout) < 0 ){
				if ( !existence_error(errno) ) 
					cvserr(0, errno, "cannot remove temp file %s", diffout);
			}
			free(diffout);
		}
		free(xrev1);
		...
		*/
		if( tmpfile1 != NULL ){
			if( CVS_UNLINK( tmpfile1 ) < 0 ){
				if( !existence_error( errno ) )
					cvserr( 0, errno, "cannot remove temp file %s", tmpfile1 );
			}
			free( tmpfile1 );
		}
		if( tmpfile2 != NULL ){
			if( CVS_UNLINK( tmpfile2 ) < 0 ){
				if( !existence_error( errno ) )
					cvserr( 0, errno, "cannot remove temp file %s", tmpfile2 );
			}
			free(tmpfile2);
		}
	}
...
int diff_exec(const char *file1, const char *file2, const char *label1,
	...
	if(preserve_perms &&
		...
		if ( lstat(file1, &sb1) < 0 ) 
			cvserr(1, errno, "cannot get file information for %s",
			        file1);
...
static void RCS_output_diff_options(int diff_argc, char * const *diff_argv,
