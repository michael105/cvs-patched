...
int checkout(int argc, char **argv){
	...
	*/
	...
	if ( cat && argc != 0 ) 
		cvserr(1, 0, "-c and -s must not get any arguments");
	...
	if ( where && pipeout ) 
		cvserr(1, 0, "-d and -p are mutually exclusive");

	if ( m_type == EXPORT ){
		if ( !tag && !date ) 
			cvserr(1, 0, "must specify a tag or date");

		if ( tag && isdigit(tag[0]) ) 
			cvserr(1, 0, "tag `%s' must be a symbolic tag", tag);
	}
...
static void build_one_dir(char *repository, char *dirpath, int sticky){
	...
	if ( isfile(CVSADM) ){
		if ( m_type == EXPORT ) 
			cvserr(1, 0, "cannot export into a working directory");
	} else if ( m_type == CHECKOUT ){
		/* I suspect that this check could be omitted.  */
		if ( !isdir(repository) ) 
			cvserr(1, 0, "there is no repository %s", repository);
...
*/
/* ARGSUSED */
static int checkout_proc(int argc, char **argv, char *where_orig, char *mwhere,
	...
	*/
	...
	if ( mfile != NULL ){
		...
		if ( mfile[strlen(mfile) - 1] == '/' ){
			cvserr(0, 0, "checkout_proc: trailing slash on mfile (%s)!",
			        mfile);
		}
...
internal error: %s doesn't start with %s in checkout_proc",
		...
		if ( build_dirs_and_chdir(head, argc <= 1) != 0 ){
			cvserr(0, 0, "ignoring module %s", omodule);
			err = 1;
			goto out;
		}
		...
		if ( !isfile(CVSADM) ){
			...
			if ( m_type == EXPORT ) 
				cvserr(1, 0, "cannot export into working directory");
			...
			if ( fncmp(repository, repos) != 0 ){
				...
				cvserr(0, 0, "existing repository %s does not match %s",
				        prepos, prepository);
				cvserr(0, 0, "ignoring module %s", omodule);
				free(repos);
				free(prepos);
...
char *emptydir_name(void){
	...
	if ( !isfile(repository) ){
		...
		if ( CVS_MKDIR(repository, 0777) < 0 ) 
			cvserr(1, errno, "cannot create %s", repository);
		(void) umask(omask);
	}
	return repository;
}
...
static int build_dirs_and_chdir(struct dir_to_build *dirs, int sticky){
	...
	while ( dirs != NULL ){
		...
		if ( CVS_CHDIR(dir) < 0 ){
			cvserr(0, errno, "cannot chdir to %s", dir);
			retval = 1;
			goto out;
		}
