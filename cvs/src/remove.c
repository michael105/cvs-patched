...
int cvsremove(int argc, char **argv){
	...
	if ( removed_files && !really_quiet ) 
		cvserr(0, 0, "use `%s commit' to remove %s permanently",
		        program_name,
...
static int remove_fileproc(void *callerdat, struct file_info *finfo){
	...
	if ( vers->ts_user != NULL ){
		existing_files++;
		if ( !quiet )
			cvserr(0, 0, "file `%s' still in working directory",
			        finfo->fullname);
		...
		if ( unlink_file(fname )  < 0
			...
			cvserr(0, errno, "cannot remove %s", CVSEXT_LOG);
		if ( !quiet )
			cvserr(0, 0, "removed `%s'", finfo->fullname);
		...
		if ( !quiet )
			cvserr(0, 0, "file `%s' already scheduled for removal",
			        finfo->fullname);
...
static Dtype remove_dirproc(void *callerdat, const char *dir,
	...
	if ( !quiet )
		cvserr(0, 0, "Removing %s", update_dir);
	return(R_PROCESS);
}
