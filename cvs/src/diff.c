...
/*  Add one of OPT or LONGOPT, and ARGUMENT, when present, to global DIFF_ARGV.
    ...
    RETURNS
     Nothing.
*/
static void add_diff_args(char opt, const char *longopt,
                           const char *argument){
	char *tmp;
	...
	*/
	while((c = getopt_long (argc, argv,
		...
		case 'L':
			if ( have_rev1_label++ )
				if ( have_rev2_label++ ){
					cvserr(0, 0, "extra -L arguments ignored");
					break;
				}
		/* Fall through.  */
		case 'C':
...
static int diff_fileproc(void *callerdat, struct file_info *finfo){
	...
	if ( diff_rev2 || diff_date2 ){
		...
		if ( (diff_rev1 || diff_date1 ) 
			...
			cvserr(0, 0, "I know nothing about %s", finfo->fullname);
			goto out;
		}
		...
		if ( !exists ){
			...
			*/
			if ( empty_files )
				...
				cvserr(0, 0, "%s is a new entry, no comparison available",
				        finfo->fullname);
				goto out;
			}
		}
		...
		if ( empty_files )
			...
			cvserr(0, 0, "%s was removed, no comparison available",
			        finfo->fullname);
			goto out;
		}
	} else{
		if ( !vers->vn_rcs && !vers->srcfile ){
			...
			if ( vers->ts_user == NULL ){
				cvserr(0, 0, "cannot find %s", finfo->fullname);
				goto out;
	...
	if ( tocvsPath ){
		...
		if ( unlink_file_dir(fname) < 0 ) 
			if ( !existence_error(errno) ) 
				cvserr(1, errno, "cannot remove %s", fname);
	...
	*/
	...
	switch(status){
	case -1:			/* fork failed */
		cvserr(1, errno, "fork failed while diffing %s",
		        vers->srcfile->path);
...
out:
	if( tocvsPath != NULL ){
		if ( unlink_file_dir(finfo->file) < 0 ) 
			if ( ! existence_error(errno) ) 
				cvserr(1, errno, "cannot remove %s", finfo->file);

		rename_file(fname, finfo->file);
		if ( unlink_file(tocvsPath) < 0 ) 
			cvserr(1, errno, "cannot remove %s", tocvsPath);
		free(fname);
	}
	...
	*/
	...
	if ( rev1_cache != NULL ){
		if ( CVS_UNLINK(rev1_cache) < 0 ) 
			cvserr(0, errno, "cannot remove %s", rev1_cache);
		free(rev1_cache);
	}
...
*/
/* ARGSUSED */
static int diff_filesdoneproc(void *callerdat, int err, const char *repos,
                               const char *update_dir, List *entries){
	return diff_errors;
}
...
*/
/* ARGSUSED */
static int diff_dirleaveproc(void *callerdat, const char *dir, int err,
                              const char *update_dir, List *entries){
	return diff_errors;
}
	...
	if ( diff_rev2 || diff_date2 ){
		...
		if ( use_rev1 == NULL || RCS_isdead(vers->srcfile, use_rev1) ){
			...
			if ( use_rev2 == NULL || RCS_isdead(vers->srcfile, use_rev2) ) 
				/*  At least in the case where DIFF_REV1 and DIFF_REV2
				    ...
				    dead?), we should be returning some kind ofcvserr(see
			...
			if ( use_rev1 != NULL ){
				...
				cvserr(0, 0, "tag %s is not in file %s", diff_rev1,
				        finfo->fullname);
			else
				cvserr(0, 0, "no revision for date %s in file %s",
				        diff_date1, finfo->fullname);
			return DIFF_ERROR;
		}
		...
		if( use_rev2 == NULL || RCS_isdead( vers->srcfile, use_rev2 ) ){
			...
			if( use_rev2 != NULL ){
				...
				cvserr(0, 0, "tag %s is not in file %s", diff_rev2,
				        finfo->fullname);
			else
				cvserr(0, 0, "no revision for date %s in file %s",
				        diff_date2, finfo->fullname);
