...
/*  If an error is pending, print it and return 1.  If not, return 0.
    ...
    Must be called only in contexts where it is OK to send output.  */
static int print_pending_error(void){
...
/*  Allocate SIZE bytes for pending_error_text and return nonzero
    if we could do it.  */
static inline int alloc_pending_internal(char **dest, size_t size){
	*dest = malloc(size);
	if ( !*dest ){
...
/*  Allocate SIZE bytes for pending_error_text and return nonzero
    if we could do it.  */
static int alloc_pending_warning(size_t size){
	if ( warning_pending() ) 
...
/*
    ...
    RETURNS
     ...
     false                Otherwise.
*/
static inline bool isProxyServer(void){
...
static void input_memory_error(struct buffer *buf){
...
/*  This function rewinds the net connection using the write proxy log file.
    ...
    RETURNS
     Nothing.
*/
static void rewind_buf_from_net(void){
	struct buffer *log;
...
/*  Is this pathname OK to recurse into when we are running as the server?
    If not, callcvserr() with a fatal error.  */
void server_pathname_check(char *path){
	...
	if ( pathname_levels(path) > max_dotdot_limit ){
		...
		cvserr(0, 0, "protocol error: `%s' contains more leading ..",
		        path);
		cvserr(1, 0, "than the %d which Max-dotdot specified",
		        max_dotdot_limit);
	}
}
...
/*  Is file or directory REPOS an absolute pathname within the
    ...
    and return 1.  */
static int outside_root(char *repos){
...
static void serve_repository(char *arg){
# ifdef PROXY_SUPPORT
	assert(!proxy_log);
# endif /* PROXY_SUPPORT */
...
/*
    ...
    enough.  Or something.
*/
static void receive_partial_file(size_t size, int file){
	while ( size > 0 ){
		int status;
		size_t nread;
		char *data;
...
static void receive_file(size_t size, char *file, int gzipped){
	int fd;
	char *arg = file;
...
*/
static int prepost_proxy_proc(const char *repository, const char *filter,
                               void *closure){
	char *cmdline;
	bool *pre = closure;
...
# endif /* SUPPORT_OLD_INFO_FMT_STRINGS */
	...
	if ( !cmdline || !strlen(cmdline) ){
		...
		if ( *pre )
			cvserr(0, 0, "preadmin proc resolved to the empty string!");
		else
			cvserr(0, 0, "postadmin proc resolved to the empty string!");
		return 1;
	}
...
static void become_proxy(void){
	...
	if ( (status = set_nonblock(buf_to_primary)) ) 
		cvserr(1, status, "failed to set nonblocking io to primary");
	if ( (status = set_nonblock(buf_to_net)) ) 
		cvserr(1, status, "failed to set nonblocking io to client");
		...
		*/
		if(buf_clientlog
			...
			while ( buf_clientlog && toread > 0 ){
				...
				if ( s == -2 ) 
					cvserr(1, ENOMEM, "Failed to read data.");
				if ( s == -1 ){
					buf_shutdown(buf_clientlog);
					buf_clientlog = NULL;
				} else if ( s )
					cvserr(1, s, "Error reading writeproxy log.");
				else{
					thispass = got;
					while ( thispass > 0 ){
						...
						*/
						if ( memcmp(data, newdata, got) ) 
							cvserr(1, 0, "Secondary out of sync with primary!");
						data += got;
						thispass -= got;
					}
					toread -= got;
...
static void serve_hostname(char *arg){
	free(hostname);
...
static void serve_localdir(char *arg){
...
/*  Process all the Notify requests that we have stored up.  Returns 0
    if successful, if not prints error message(viacvserr()) and
    returns negative value.  */
static int server_notify(void){
	...
	while ( notify_list != NULL ){
		if ( CVS_CHDIR(notify_list->dir) < 0 ){
			cvserr(0, errno, "cannot change to %s", notify_list->dir);
			return -1;
		}
...
error_return:
	...
	switch(arg[1]){
	case 'l':
		cvserr(0, 0, "WARNING: global `-l' option ignored.");
		break;
	case 'n':
		noexec = 1;
		logoff = 1;
...
static void serve_kerberos_encrypt( char *arg ){
#     ifdef PROXY_SUPPORT
	assert(!proxy_log);
#     endif /* PROXY_SUPPORT */
...
static void serve_gssapi_encrypt( char *arg ){
#     ifdef PROXY_SUPPORT
	assert(!proxy_log);
#     endif /* PROXY_SUPPORT */
...
static void serve_gssapi_authenticate(char *arg){
#   ifdef PROXY_SUPPORT
	assert(!proxy_log);
#   endif /* PROXY_SUPPORT */
...
static void protocol_memory_error(struct buffer *buf){
	cvserr(1, ENOMEM, "Virtual memory exhausted");
}
...
*/
static bool check_command_valid_p(char *cmd_name){
		...
		*/
		...
		if ( fp == NULL ){
			if ( !existence_error(errno) ){
				...
				cvserr(0, errno, "cannot open %s", fname);
				free(fname);
				return false;
			}
...
static void do_cvs_command(char *cmd_name, int (*command) (int, char **)){
	/*
...
# endif /* SERVER_FLOWCONTROL */
	...
	if ( command_pid == 0 ){
		...
		if ( dup2 (dev_null_fd, STDIN_FILENO) < 0 ) 
			cvserr(1, errno, "can't set up pipes");
...
# ifdef SERVER_FLOWCONTROL
		...
		/*  FIXME: No point in printing an error message withcvserr(),
	...
	while ( paused ){
		...
		if ( FD_ISSET(flowcontrol_pipe[0], &fds) ){
			...
			/*  This assumes that we are using BSD or POSIX nonblocking
			    ...
			    nothing to read.  */
			if ( got == 0 ) 
				cvserr(1, 0, "flow control EOF");
			if ( got < 0 && ! blocking_error(errno) ){
				cvserr(1, errno, "flow control read failed");
			}
		}
	}
}
...
static void new_entries_line(void){
	if ( entries_line ){
...
static void checked_in_response(const char *file, const char *update_dir,
	...
	if ( supported_response("Mode") ){
		...
		if ( stat(file, &sb) < 0 ){
			...
			if ( !existence_error(errno) ) 
				cvserr(0, errno, "cannot stat %s", file);
		} else{
...
static void serve_rls(char *arg){
...
static void serve_edit(char *arg){
...
CVS server internal error: no mode in server_updated");
			...
			if ( stat(finfo->file, &sb) < 0 ){
				...
				cvserr(1, errno, "reading %s", finfo->fullname);
			}
			size = sb.st_size;
			if ( mode == (mode_t) -1 ){
...
CVS server internal error: unhandled case in server_updated");
				...
				if ( status == -2 ) 
					...
					cvserr(1, ferror(f) ? errno : 0, "reading %s",
					        finfo->fullname);
				if ( fclose(f) == EOF ) 
					cvserr(1, errno, "reading %s", finfo->fullname);
			}
		...
		*/
		if((updated == SERVER_UPDATED
			...
			if ( CVS_UNLINK(finfo->file) < 0 ) 
				cvserr(0, errno, "cannot remove temp file for %s",
				        finfo->fullname);
		}
...
static int template_proc(const char *repository, const char *template,
                          void *closure){
	FILE *fp;
	char buf[1024];
	size_t n;
	...
	if ( fp == NULL ){
		cvserr(0, errno, "Couldn't open rcsinfo template file %s",
		        template);
		return 1;
	}
	if ( fstat(fileno (fp), &sb) < 0 ){
		cvserr(0, errno, "cannot stat rcsinfo template file %s",
		        template);
		return 1;
	}
	sprintf (buf, "%ld\n", (long) sb.st_size);
	...
	while ( !feof(fp) ){
		...
		if ( ferror(fp) ){
			cvserr(0, errno, "cannot read rcsinfo template file %s",
			        template);
			(void) fclose(fp);
			return 1;
		}
	...
	buf_send_counted(protocol);
	if ( fclose(fp) < 0 ) 
		cvserr(0, errno, "cannot close rcsinfo template file %s",
		        template);
	return 0;
}
...
static void serve_gzip_contents(char *arg){
	int level;
	bool forced = false;
...
static int expand_proc(int argc, char **argv, char *where, char *mwhere,
                        char *mfile, int shorten, int local_specified, char *omodule, char *msg){
	int i;
	char *dir = argv[0];
...
/*  Decide if we should redirect the client to another server.
    ...
    RETURNS
     Nothing.
*/
static void serve_command_prep(char *arg){
...
/*  Save a referrer, potentially for passing to hook scripts later.
    ...
    RETURNS
     Nothing.
*/
static void serve_referrer(char *arg){
	if ( error_pending() )  return;
		...
		if ( buf_to_net != NULL ){
			...
			*/
			...
			if ( buf_from_net ){
				...
				if ( status != 0 ) 
					cvserr(0, status, "shutting down buffer from client");
				buf_free(buf_from_net);
				buf_from_net = NULL;
			}
			/* SIG_endCrSect(); */
...
#endif /* HAVE_SYSLOG_H */
		cvserr(0, 0, "Dying gasps received from client.");
	}
...
static void switch_to_user(const char *cvs_username, const char *username){
	struct passwd *pw;
#ifdef HAVE_PAM
	int retval;
...
*/
static int check_repository_password(char *username, char *password,
                                      char *repository, char **host_user_ptr){
	int retval = 0;
	FILE *fp;
	char *filename;
	char *linebuf = NULL;
	...
	*/
	...
	if ( fp == NULL ){
		if ( !existence_error(errno) ) 
			cvserr(0, errno, "cannot open %s", filename);
		free(filename);
		return 0;
	}
	...
	if ( ferror(fp) ) 
		cvserr(0, errno, "cannot read %s", filename);
	if ( fclose(fp) < 0 ) 
		cvserr(0, errno, "cannot close %s", filename);
...
static int check_pam_password(char **username, char *password){
	int retval, err;
...
static int check_system_password(char *username, char *password){
...
static char *check_password(char *username, char *password, char *repository){
	int rc;
	char *host_user = NULL;
...
static void pserver_read_line(char **tmp, size_t *tmp_len){
	int status;
...
# endif /* HAVE_SYSLOG_H */
...
#else
		cvserr(1, 0,
		        "GSSAPI authentication not supported by this server");
#endif
	} else
		cvserr(1, 0, "bad auth protocol start: %s", tmp);

#ifndef AUTH_SERVER_SUPPORT

	cvserr(1, 0,
	        "Password authentication not supported by this server");
	...
	if(strcmp (tmp,
	        ...
	        != 0){
		cvserr(1, 0, "bad auth protocol end: %s", tmp);
	}
	free(tmp);

	if ( !root_allow_ok(repository) ){
		cvserr(1, 0, "%s: no such repository", repository);
# ifdef HAVE_SYSLOG_H
...
static void gserver_authenticate_connection(void){
	...
	if ( !hn )
		cvserr(1, 0, "can't get canonical hostname for `%s': %s",
	...
	if(gss_import_name (&stat_min, &tok_in, GSS_C_NT_HOSTBASED_SERVICE,
		...
		cvserr(1, 0, "could not import GSSAPI service name %s", buf);
	...
	if(gss_acquire_cred (&stat_min, server_name, 0, GSS_C_NULL_OID_SET,
		...
		cvserr(1, 0, "could not acquire GSSAPI server credentials");
	...
	if ( fread(buf, 1, 2, stdin) != 2 ) 
		cvserr(1, errno, "read of length failed");
	...
	if ( fread(credbuf, 1, nbytes, stdin) != nbytes ) 
		cvserr(1, errno, "read of data failed");
	...
	if(gss_accept_sec_context (&stat_min,
	        ...
	        != GSS_S_COMPLETE){
		cvserr(1, 0, "could not verify credentials");
	}
...
static int krb_encrypt_input( void *fnclosure, const char *input, char *output,
                              int size ){
...
static int krb_encrypt_output( void *fnclosure, const char *input, char *output,
...
void cvs_output_binary(char *str, size_t len){
#ifdef SERVER_SUPPORT
...
this client does not support writing binary files to stdout");
...
#endif
	{
		...
		if ( oldmode < 0 ) 
			cvserr(0, errno, "failed to setmode on stdout");
#endif
...
void cvs_outerr(const char *str, size_t len){
	if ( len == 0 ) 
		len = strlen(str);
#ifdef SERVER_SUPPORT
	if ( error_use_protocol ){
