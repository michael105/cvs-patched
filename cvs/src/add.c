...
int add(int argc, char **argv){
	...
	for(i = 0; i < argc; i++){
		...
		if ( strcmp(argv[i], "." )  == 0
			...
			if ( !quiet )
				cvserr(0, 0, "cannot add special file `%s'; skipping",
				        argv[i]);
			skip_file = 1;
		} else{
			char *p;
				...
				*/
				...
				if ( strncmp(repository, current_parsed_root->directory, cvsroot_len )  == 0
				        ...
				        && strcmp(repository + cvsroot_len + sizeof CVSROOTADM + 1,
				                   CVSNULLREPOS) == 0)
					cvserr(1, 0, "cannot add to `%s'", repository);
		...
		*/
		...
		if ( p == filename ){
			...
			if ( CVS_CHDIR(finfo.update_dir) < 0 ) 
				cvserr(1, errno, "could not chdir to `%s'",
				        finfo.update_dir);
		}
		...
		if ( vers->vn_user == NULL ){
			...
			if ( vers->vn_rcs == NULL ){
					...
					*/
					...
					if ( isdir(dname) ){
						cvserr(0, 0,
						        ...
						        finfo.fullname);
						cvserr(0, 0, "`%s' already exists in the repository",
						        dname);
						cvserr(1, 0, "invalid filename overlap");
					}
					free(dname);
				...
				if ( isdir(finfo.file ) 
					...
					cvserr(1, 0, "same name already exists in the repository.");
				} else{
					if ( vers->nonbranch ){
						cvserr(0, 0,
						        "cannot add file on non-branch tag `%s'",
						...
						if ( vers->ts_user == NULL ){
							...
							*/
							...
							xchmod(finfo.file, 1);
							if ( status != 0 ){
								cvserr(0, 0, "Failed to resurrect revision %s",
								        prev);
								err++;
							} else{
				...
				/*
				    ...
				    added it
				*/
				cvserr(0, 0, "`%s' added independently by second party",
				        finfo.fullname);
				err++;
			}
			...
			*/
			if ( !quiet )
				cvserr(0, 0, "`%s' has already been entered",
				        finfo.fullname);
			err++;
					...
					*/
					...
					if ( status != 0 ){
						...
						*/
						...
						write_letter(&finfo, 'U');
						if ( !quiet )
							cvserr(0, 0, "`%s', version %s, resurrected",
...
`%s' should be removed and is still there(or is back again)", finfo.fullname);
			...
			if ( !quiet )
				cvserr(0, 0, "`%s' already exists, with version number %s",
				        finfo.fullname,
				        vers->vn_user);
			err++;
		}
...
skip_this_file:
	...
	if ( added_files && !really_quiet ) 
		cvserr(0, 0, "use `%s commit' to add %s permanently",
		        program_name,
...
static int add_directory(struct file_info *finfo){
	...
	if ( fncmp(dir, CVSADM) == 0 ){
		cvserr(0, 0, "cannot add a `%s' directory", CVSADM);
		return 1;
	}
	...
	if ( save_cwd(&cwd) ){
		cvserr(0, errno, "Failed to save current directory.");
		return 1;
	}
	if ( CVS_CHDIR(dir) < 0 ){
		cvserr(0, errno, "cannot chdir to %s", finfo->fullname);
		return 1;
	}
	if ( !server_active && isfile(CVSADM) ){
		cvserr(0, 0, "%s/%s already exists", finfo->fullname,
		        CVSADM);
		goto out;
	}
	...
	if ( isfile(rcsdir) && !isdir (rcsdir) ){
		cvserr(0, 0, "%s is not a directory; %s not added", rcsdir,
		        finfo->fullname);
		goto out;
	}
	...
	if ( !isdir(rcsdir) ){
		...
		if ( !noexec ){
			...
			if ( CVS_MKDIR(rcsdir, 0777) < 0 ){
				cvserr(0, errno, "cannot mkdir %s", rcsdir);
				(void) umask(omask);
				goto out;
			}
			(void) umask(omask);
...
static int build_entry(const char *repository, const char *user,
	...
	*/
	...
	if ( message && fputs(message, fp) == EOF ) 
		cvserr(1, errno, "cannot write to %s", fname);
	if ( fclose(fp) == EOF ) 
		cvserr(1, errno, "cannot close %s", fname);
	free(fname);
