...
static void recv_bytes(int sock, char *buf, int need){
	while ( need > 0 ){
		int got;

		got = recv(sock, buf, need, 0);
		if ( got <= 0 ) 
			cvserr(1, 0, "recv() from server %s: %s",
...
*/
#define BUFSIZE 1024
int connect_to_gserver(cvsroot_t *root, int sock, struct hostent *hostinfo){
	char *str;
	char buf[BUFSIZE];
	...
	if ( send(sock, str, strlen (str), 0) < 0 ) 
		cvserr(1, 0, "cannot send: %s", SOCK_STRERROR (SOCK_ERRNO));
	...
	do{
		...
		if ( stat_maj != GSS_S_COMPLETE && stat_maj != GSS_S_CONTINUE_NEEDED ){
			...
			cvserr(0, 0, "GSSAPI authentication failed: %s",
			        (const char *) tok_out.value);

			message_context = 0;
			gss_display_status(&new_stat_min, stat_min, GSS_C_MECH_CODE,
			                    GSS_C_NULL_OID, &message_context, &tok_out);
			cvserr(1, 0, "GSSAPI authentication failed: %s",
			        (const char *) tok_out.value);
		}
		...
		if ( tok_out.length == 0 ){
			...
			if ( need > sizeof buf ){
				...
				if ( buf[total - 1] == '\n' ) 
					buf[total - 1] = '\0';
				cvserr(1, 0, "error from server %s: %s", root->hostname,
				        buf);
			}
...
static int cvs_gssapi_wrap_input(void *fnclosure, const char *input,
                                  char *output,
                                  size_t size){
...
static int cvs_gssapi_wrap_output(void *fnclosure, const char *input,
                                   char *output,
...
void initialize_gssapi_buffers(struct buffer **to_server_p,
