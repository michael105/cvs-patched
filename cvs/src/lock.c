		...
		*q++ = '/';
		...
		if ( stat(retval, &sb) < 0 ){
			...
			if ( !existence_error(errno) ) 
				cvserr(1, errno, "cannot stat directory %s", retval);
		} else{
			if ( S_ISDIR(sb.st_mode) ) 
				goto created;
			else
				cvserr(1, 0, "%s is not a directory", retval);
		}
		...
		if ( stat(config->lock_dir, &sb) < 0 ) 
			cvserr(1, errno, "cannot stat %s", config->lock_dir);
		new_mode = sb.st_mode;
			...
			if ( ISSLASH(*p) ){
				...
				if ( !ISSLASH(q[p - short_repos - 1] ) 
					...
					if ( saved_errno != EEXIST ) 
						cvserr(1, errno, "cannot make directory %s", retval);
					else{
						if ( stat(retval, &sb) < 0 ) 
							cvserr(1, errno, "cannot stat %s", retval);
						new_mode = sb.st_mode;
					}
				}
				++p;
...
*/
static void remove_lock_files(struct lock *lock, bool free_repository){
	...
	*/
	if ( lock->file1 ){
		...
		if ( CVS_UNLINK(tmp) < 0 && ! existence_error (errno) ) 
			cvserr(0, errno, "failed to remove lock %s", tmp);
		free(tmp);
	}
#ifdef LOCK_COMPATIBILITY
	if ( lock->file2 ){
...
*/
static void set_readlock_name(void){
	if ( readlock == NULL ){
		readlock = Xasprintf (
...
int Reader_Lock(char *xrepository){
	...
	if ( set_lock(&global_readlock, 1) != L_OK ){
		cvserr(0, 0, "failed to obtain dir lock in repository `%s'",
		        xrepository);
		if ( readlock != NULL ) 
			free(readlock);
		readlock = NULL;
...
*/
static int lock_exists(const char *repository, const char *filepat,
                        const char *ignore){
	char *lockdir;
	char *line;
	DIR *dirp;
	...
	do{
		if ( (dirp = CVS_OPENDIR(lockdir)) == NULL ) 
			cvserr(1, 0, "cannot open directory %s", lockdir);
			...
			if ( CVS_FNMATCH(filepat, dp->d_name, 0) == 0 ){
				...
				*/
				...
				if ( stat(line, &sb) != -1 ){
					...
					*/
					if ( !existence_error(errno) ) 
						cvserr(0, errno, "cannot stat %s", line);
				}
				errno = 0;
				free(line);
				ret = 1;
		...
		if ( errno != 0 ) 
			cvserr(0, errno, "error reading directory %s", repository);
...
/*
    ...
    See lock_exists() for argument detail.
*/
static int promotable_exists(const char *repository){
	...
	if ( status == L_OK ){
		...
		if ( (fp = CVS_FOPEN (lock->file1, "w+")) == NULL || fclose (fp) == EOF ){
			...
			if ( CVS_UNLINK(lock->file1) < 0 && ! existence_error (errno) ) 
				cvserr(0, errno, "failed to remove lock %s", lock->file1);
		...
		*/
		...
		if ( (fp = CVS_FOPEN (lock->file2, "w+")) == NULL || fclose (fp) == EOF ){
			...
			if ( CVS_UNLINK(lock->file2) < 0 && ! existence_error (errno))
				cvserr(0, errno, "failed to remove lock %s", lock->file2);
...
/*
    ...
    RETURNS
     0 for no error.
*/
static int set_promotablelock_proc(Node *p, void *closure){
...
static int lock_list_promotably(List *list){
	char *wait_repos;
...
Attempting to write to a read-only filesystem is not allowed.");
	...
	for(;;){
		...
		case L_ERROR:		/* Real Error */
			...
			cvserr(0, 0, "lock failed - giving up");
			return 1;
...
after_sig_unblock:
		...
		if ( stat(masterlock, &sb) < 0 ){
			...
			cvserr(0, errno, "couldn't stat lock directory `%s'",
			        masterlock);
			status = L_ERROR;
			goto done;
		}
...
static void clear_lock(struct lock *lock){
	...
	if ( lock->lockdir ){
		if ( CVS_RMDIR(lock->lockdir) < 0 ) 
			cvserr(0, errno, "failed to remove lock dir `%s'",
			        lock->lockdir);
		free(lock->lockdir);
		lock->lockdir = NULL;
	}
...
*/
/* ARGSUSED */
static int lock_filesdoneproc(void *callerdat, int err, const char *repository,
...
void lock_tree_promotably(int argc, char **argv, int local, int which,
	...
	*/
	...
	if ( lock_list_promotably(lock_tree_list) != 0 ) 
		cvserr(1, 0, "lock failed - giving up");
}
		...
		for(;;){
			...
			if ( (fp = CVS_FOPEN(global_writelock.file1, "w+") )  == NULL
				...
				if ( CVS_UNLINK(global_writelock.file1 )  < 0
					...
					cvserr(0, errno, "failed to remove write lock %s",
...
/*  This is the internal implementation behind history_lock & val_tags_lock.  It
    ...
    RETURNS
     ...
     false, on error
*/
static inline int internal_lock(struct lock *lock, const char *xrepository){
	/* remember what we're locking(for Lock_Cleanup) */
...
*/
int history_lock(const char *xrepository){
...
*/
int val_tags_lock(const char *xrepository){
