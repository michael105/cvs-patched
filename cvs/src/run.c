...
void run_arg_free_p(int argc, char **argv){
	int i;
	for(i = 0; i < argc; i++)
		free(argv[i]);
}
...
void run_add_arg_p(int *iargc, size_t *iarg_allocated, char ***iargv,
                    const char *s){
...
int run_exec(const char *stin, const char *stout, const char *sterr,
	...
	if ( stin && (shin = open(stin, O_RDONLY)) == -1 ){
		rerrno = errno;
		cvserr(0, errno, "cannot open %s for reading (prog %s)",
		        stin, run_argv[0]);
		goto out0;
	}
	if ( stout && (shout = open(stout, mode_out, 0666)) == -1 ){
		rerrno = errno;
		cvserr(0, errno, "cannot open %s for writing (prog %s)",
		        stout, run_argv[0]);
		goto out1;
	}
	...
	if ( pid == 0 ){
		...
		*/
		if ( !strcmp(run_argv[0], Editor) && setegid (getgid ()) ){
			cvserr(0, errno, "cannot set egid to gid");
			_exit(127);
		}
#endif
		...
		cvserr(0, errno, "cannot exec %s", run_argv[0]);
		_exit(127);
	} else if ( pid == -1 ){
		rerrno = errno;
		goto out;
...
void run_print(FILE *fp){
	...
	if ( fp == stderr ) 
		...
		cvserr(1, 0, "internal error: bad argument to run_print");
...
FILE *run_popen(const char *cmd, const char *mode){
...
static void work_around_openssh_glitch(void){
	pid_t pid;
	int stderr_pipe[2];
	struct stat sb;
	...
	if ( pipe(stderr_pipe) < 0 ) 
		cvserr(1, errno, "cannot create pipe");
	pid = fork();
	if ( pid < 0 ) 
		cvserr(1, errno, "cannot fork");
	if ( pid != 0 ){
		...
		if ( close(stderr_pipe[1]) < 0 ) 
			cvserr(1, errno, "cannot close pipe");
		...
		while ( (inbytes = read(stderr_pipe[0], buf, sizeof buf)) != 0 ){
			...
			if ( inbytes < 0 ){
				if ( errno == EINTR ) 
					continue;
				cvserr(1, errno, "reading from pipe");
			}
		...
		while ( (w = waitpid(pid, &status, 0)) == -1 && errno == EINTR ) 
			continue;
		if ( w < 0 ) 
			cvserr(1, errno, "waiting for child");
		if ( !WIFEXITED(status) ){
			if ( WIFSIGNALED(status) ) 
				raise(WTERMSIG (status));
			cvserr(1, errno, "child did not exit cleanly");
		}
	...
	if ( stderr_pipe[1] != STDERR_FILENO ){
		if ( dup2 (stderr_pipe[1], STDERR_FILENO) < 0 ) 
			cvserr(1, errno, "cannot dup2 pipe");
		if ( close(stderr_pipe[1]) < 0 ) 
			cvserr(1, errno, "cannot close pipe");
	}
}
...
int piped_child(char *const *command, int *tofdp, int *fromfdp,
                 bool fix_stderr){
	int pid;
	int to_child_pipe[2];
	...
	if ( pid == 0 ){
		...
		cvserr(1, errno, "cannot exec %s", command[0]);
	}
...
int run_piped(int *tofdp, int *fromfdp){
	run_add_arg(NULL);
...
void close_on_exec(int fd){
	...
	if ( fcntl(fd, F_SETFD, 1) == -1 ) 
		cvserr(1, errno, "can't set close-on-exec flag on %d", fd);
#endif
}
