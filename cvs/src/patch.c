...
int patch(int argc, char **argv){
	...
	if ( toptwo_diffs && patch_short ) 
		cvserr(1, 0, "-t and -s options are mutually exclusive");
	...
	if(!toptwo_diffs && (date1 == NULL && date2 == NULL &&
	                      rev1 == NULL && rev2 == NULL))
		cvserr(1, 0, "must specify at least one revision/date!");
	if ( date1 != NULL && date2 != NULL ) 
		if ( RCS_datecmp(date1, date2) >= 0 ) 
			cvserr(1, 0, "second date must come after first date!");
...
*/
/* ARGSUSED */
static int patch_proc(int argc, char **argv, char *xwhere, char *mwhere,
                       char *mfile,
...
static int patch_fileproc(void *callerdat, struct file_info *finfo){
	...
	if ( toptwo_diffs ){
		...
		*date1 = '\0';
		if ( RCS_getrevtime(rcsfile, vers_head, date1, 1) == (time_t)-1 ){
			if ( !really_quiet )
				cvserr(0, 0, "cannot find date in rcs file %s revision %s",
				        rcs, vers_head);
			ret = 1;
			goto out2;
		}
	...
	*/
	if ( (fp1 = cvs_temp_file(&tmpfile1)) == NULL ){
		cvserr(0, errno, "cannot create temporary file %s",
		        tmpfile1);
		ret = 1;
		goto out;
	} else if ( fclose(fp1) < 0 ) 
		cvserr(0, errno, "warning: cannot close %s", tmpfile1);
	if ( (fp2 = cvs_temp_file(&tmpfile2)) == NULL ){
		...
		cvserr(0, errno, "warning: cannot close %s", tmpfile2);
	if ( (fp3 = cvs_temp_file(&tmpfile3)) == NULL ){
		cvserr(0, errno, "cannot create temporary file %s",
		        tmpfile3);
		ret = 1;
		goto out;
	} else if ( fclose(fp3) < 0 ) 
		cvserr(0, errno, "warning: cannot close %s", tmpfile3);
	...
	case -1:			/* fork/wait failure */
		cvserr(1, errno, "fork for diff failed on %s", rcs);
		break;
...
failed to read diff file header %s for %s: end of file", tmpfile3, rcs);
			...
			ret = 1;
			if ( fclose(fp) < 0 ) 
				cvserr(0, errno, "error closing %s", tmpfile3);
			goto out;
		}
		if ( !unidiff ){
			if ( strncmp(line1, "*** ", 4 )  != 0 ||
				...
				cvserr(0, 0, "invalid diff header for %s", rcs);
				ret = 1;
				if ( fclose(fp) < 0 ) 
					cvserr(0, errno, "error closing %s", tmpfile3);
				goto out;
			...
			if ( strncmp(line1, "--- ", 4 )  != 0 ||
			        ...
			        (cp1 = strchr(line1, '\t')) == NULL ||
			        (cp2 = strchr  (line2, '\t')) == NULL){
				cvserr(0, 0, "invalid unidiff header for %s", rcs);
				ret = 1;
				if ( fclose(fp) < 0 ) 
					cvserr(0, errno, "error closing %s", tmpfile3);
				goto out;
		...
		if ( line_length < 0 && !feof(fp) ) 
			cvserr(0, errno, "cannot read %s", tmpfile3);

		if ( fclose(fp) < 0 ) 
			cvserr(0, errno, "cannot close %s", tmpfile3);
		free(file1);
		free(file2);
		break;
	default:
		cvserr(0, 0, "diff failed for %s", finfo->fullname);
	}
out:
	if ( line1 )
		free(line1);
	...
	if ( CVS_UNLINK(tmpfile1) < 0 ) 
		cvserr(0, errno, "cannot unlink %s", tmpfile1);
	if ( CVS_UNLINK(tmpfile2) < 0 ) 
		cvserr(0, errno, "cannot unlink %s", tmpfile2);
	if ( CVS_UNLINK(tmpfile3) < 0 ) 
		cvserr(0, errno, "cannot unlink %s", tmpfile3);
	free(tmpfile1);
	free(tmpfile2);
	free(tmpfile3);
...
*/
/* ARGSUSED */
static Dtype patch_dirproc(void *callerdat, const char *dir, const char *repos,
                            const char *update_dir, List *entries){
	if ( !quiet )
		cvserr(0, 0, "Diffing %s", update_dir);
	return R_PROCESS;
}
...
static RETSIGTYPE patch_cleanup(int sig){
	...
	if ( tmpfile1 != NULL ){
		if ( unlink_file(tmpfile1 )  < 0
			...
			cvserr(0, errno, "cannot remove %s", tmpfile1);
		free(tmpfile1);
	}
	if ( tmpfile2 != NULL ){
		if ( unlink_file(tmpfile2 )  < 0
		        && !existence_error(errno))
			cvserr(0, errno, "cannot remove %s", tmpfile2);
		free(tmpfile2);
	}
	if ( tmpfile3 != NULL ){
		if ( unlink_file(tmpfile3 )  < 0
		        && !existence_error(errno))
			cvserr(0, errno, "cannot remove %s", tmpfile3);
		free(tmpfile3);
	}
	tmpfile1 = tmpfile2 = tmpfile3 = NULL;
	...
	if ( sig != 0 ){
		...
		cvserr(0, 0, "received %s signal", name);
	}
}
