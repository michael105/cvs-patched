...
static int add_entries_proc(Node *node, void *closure){
	Node *fnode;
...
List *Find_Names(char *repository, int which, int aflag, List **optentries){
	...
	if ( (which & W_REPOS) && repository && !isreadable(CVSADM_ENTSTAT) ){
		...
		if ( find_rcs(repository, files) != 0 ){
			cvserr(0, errno, "cannot open directory %s",
		...
		if ( which & W_ATTIC ){
			...
			if ( find_rcs(dir, files )  != 0
				...
				cvserr(1, errno, "cannot open directory %s",
...
List *Find_Directories(char *repository, int which, List *entries){
	...
	if ( which & W_LOCAL ){
		...
		if ( tmpentries != NULL && (sdtp == NULL || sdtp->subdirs) ) 
			...
			if ( find_dirs(".", dirlist, 1, tmpentries) != 0 ) 
				cvserr(1, errno, "cannot open current directory");
			if ( tmpentries != NULL ){
	...
	if ( (which & W_REPOS) && repository ){
		...
		if ( find_dirs(repository, dirlist, 0, entries) != 0 ) 
			cvserr(1, errno, "cannot open directory %s", repository);
...
/*  Finds all the files matching PAT.  If DIR is NULL, PAT will be interpreted
    ...
    ERRORS
     ...
     message will be printed viacvserr(0, errno, ...).
*/
...
*/
static int strip_rcsext(Node *p, void *closure){
