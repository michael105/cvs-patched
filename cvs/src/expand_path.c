...
static void variable_delproc(Node *node){
	free(node->data);
}
...
void variable_set(char *nameval){
	...
	while ( isalnum((unsigned char) *p) || *p == '_' ) 
		++p;
	if ( *p != '=' ) 
		cvserr(1, 0, "invalid character in user variable name in %s",
		        nameval);
	if ( p == nameval ) 
		cvserr(1, 0, "empty user variable name in %s", nameval);
	...
	++p;
	if ( strchr(p, '\012') ) 
		cvserr(1, 0, "linefeed in user variable value in %s",
		        nameval);
...
/*  Expand variable NAME into its contents, per the rules above.
    ...
    ERRORS
     ...
     found.
*/
static const char *expand_variable(const char *name, const char *cvsroot,
	...
	if ( !strcmp(name, CVSROOT_ENV) ) 
		...
		if ( line )
			cvserr(0, 0, "%s:%d: no such internal variable $%s",
			        file, line, name);
		else
			cvserr(0, 0, "%s: no such internal variable $%s",
			        file, name);
		return NULL;
	} else if ( name[0] == '=' ){
		Node *node;
		...
		if ( !node ){
			if ( line )
				cvserr(0, 0, "%s:%d: no such user variable ${%s}",
				        file, line, name);
			else
				cvserr(0, 0, "%s: no such user variable ${%s}",
				        file, name);
			return NULL;
		}
		return node->data;
		...
		if ( line )
			cvserr(0, 0, "%s:%d: unrecognized variable syntax %s",
			        file, line, name);
		else
			cvserr(0, 0, "%s: unrecognized variable syntax %s",
			        file, name);
		return NULL;
	}
}
...
/*  This routine will expand the pathname to account for ~ and $
    ...
    viacvserr() and NULL is returned.  FILE and LINE are the filename
	...
	*/
	if ( mybuf[s] == '~' ){
		...
		if ( p == d ) 
			...
			if ( ps == NULL ){
				if ( line )
					cvserr(0, 0, "%s:%d: no such user %s",
					        file, line, buf + d);
				else
					cvserr(0, 0, "%s: no such user %s", file, buf + d);
				goto error_exit;
			}
			e = ps->pw_dir;
#endif
		...
		if ( !e )
			cvserr(1, 0, "cannot find home directory");
