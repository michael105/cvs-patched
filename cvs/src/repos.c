	...
	if ( fpin == NULL ){
		...
		if ( existence_error(save_errno) ){
			...
			*/
			...
			cvserr(0, 0, "besides CVS administrative files.");
			cvserr(1, 0, "*PANIC* administration files missing!");
		}

		cvserr(1, save_errno, "cannot open `%s'", tmp);
	}

	if ( getline(&repos, &repos_allocated, fpin) < 0 ){
		/* FIXME: should be checking for end of file separately.  */
		cvserr(0, 0, "in directory `%s':", xupdate_dir);
		cvserr(1, errno, "cannot read `%s'", CVSADM_REP);
	}
	if ( fclose(fpin) < 0 ) 
		cvserr(0, errno, "cannot close `%s'", tmp);
	free(tmp);
	...
	*/
	if ( !ISABSOLUTE(repos) ){
		...
		if ( current_parsed_root == NULL ){
			...
			cvserr(0, 0, "or specify the '-d' option to `%s'.",
			        program_name);
			cvserr(1, 0, "invalid repository setting");
		}
		if ( pathname_levels(repos) > 0 ){
			cvserr(0, 0, "in directory `%s':", xupdate_dir);
			cvserr(0, 0,
			        "`..'-relative repositories are not supported.");
			cvserr(1, 0, "invalid source repository");
		}
